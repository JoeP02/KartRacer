...
FEOSConfigurationWriter::FEOSLoadedConfigFile FEOSConfigurationWriter::GetConfigFile(const FString &Path)
{
    ...
    for (const auto &KnownFilename : GConfig->GetFilenames())
    {
        ...
        {
            bool bImpactedByThisFile = false;
...
#else
    FConfigFile *File = Redpoint::EOS::Config::FEngineConfigHelpers::FindByName(Path, true);
    checkf(File != nullptr, TEXT("Redpoint::EOS::Config::FEngineConfigHelpers::FindByName must not return nullptr"));
    FConfigFile *SavedFile = nullptr;
#endif
    ...
    return FEOSLoadedConfigFile(File, SavedFile, this->EOSLoadedFiles[Path].ToSharedRef());
}
    ...
    FName Platform)
{
    ...
    this->FilesQueuedForModify.Add(FQueuedFileToModify(File, Platform));
    this->QueuedDelegates.Add(FQueuedDelegate::CreateLambda([this, File, Platform, Section, Key, EnumValue]() {
        FEOSLoadedConfigFile LCF = GetConfigFile(EOSConfiguration::GetFilePath(File, Platform));
        if (!LCF.F)
        {
            UE_LOG(LogRedpointEOSEditor, Error, TEXT("GetConfigFile failed to load file during delegate callback."));
            return;
        }
        ...
        if (LCF.SF)
        {
            LCF.SF->SetString(*Section, *Key, *EnumValue);
        }
    ...
    FName Platform)
{
    ...
    this->FilesQueuedForModify.Add(FQueuedFileToModify(File, Platform));
    this->QueuedDelegates.Add(FQueuedDelegate::CreateLambda([this, File, Platform, Section, Key, Value]() {
        FEOSLoadedConfigFile LCF = GetConfigFile(EOSConfiguration::GetFilePath(File, Platform));
        if (!LCF.F)
        {
            UE_LOG(LogRedpointEOSEditor, Error, TEXT("GetConfigFile failed to load file during delegate callback."));
        ...
        if (LCF.SF)
        {
            LCF.SF->SetString(*Section, *Key, Value ? TEXT("True") : TEXT("False"));
        }
    ...
    FName Platform)
{
    ...
    this->QueuedDelegates.Add(FQueuedDelegate::CreateLambda([this, File, Platform, Section, Key, Value]() {
        ...
        if (LCF.SF)
        {
            LCF.SF->SetString(*Section, *Key, *Value);
        }
        LCF.Info->Upserts.Add(FSectionKey(Section, Key), FEOSSetting(Value));
        LCF.Info->bIsModified = true;
    }));
    ...
    FName Platform)
{
    ...
    this->QueuedDelegates.Add(FQueuedDelegate::CreateLambda([this, File, Platform, Section, Key, Value]() {
        ...
        if (LCF.SF)
        {
            FConfigSection *SecSF = LCF.SF->Find(Section);
            if (SecSF)
            {
                SecSF->Remove(*Key);
    ...
    FName Platform)
{
    ...
    this->QueuedDelegates.Add(
        FQueuedDelegate::CreateLambda([this, File, Platform, Section, Key, ExistingValue, Value]() {
            FEOSLoadedConfigFile LCF = GetConfigFile(EOSConfiguration::GetFilePath(File, Platform));
            if (!LCF.F)
            {
                UE_LOG(
            ...
            if (LCF.SF)
            {
                ...
                if (SecSF == nullptr)
                {
                    SecSF = &LCF.SF->Add(Section, FConfigSection());
                }
                for (const auto &Entry : Value)
                {
                    if (!ExistingValue.Contains(Entry))
                    {
                        SecSF->Add(*FString::Printf(TEXT("+%s"), *Key), Entry);
                    }
                }
            }
    ...
    FName Platform)
{
    ...
    this->FilesQueuedForModify.Add(FQueuedFileToModify(File, Platform));
    this->QueuedDelegates.Add(FQueuedDelegate::CreateLambda([this, File, Platform, Section, Key]() {
        FEOSLoadedConfigFile LCF = GetConfigFile(EOSConfiguration::GetFilePath(File, Platform));
        if (!LCF.F)
        {
            UE_LOG(LogRedpointEOSEditor, Error, TEXT("GetConfigFile failed to load file during delegate callback."));
        ...
        if (!Sec)
        {
            UE_LOG(LogRedpointEOSEditor, Error, TEXT("FindOrAddSection failed to load file during delegate callback."));
            return;
        }
        ...
        if (LCF.SF)
        {
            FConfigSection *SecSF = LCF.SF->FindOrAddSection(Section);
            if (SecSF)
            {
                SecSF->Remove(*Key);
            }
...
FEOSConfigurationWriter::EFlushChangesResult FEOSConfigurationWriter::FlushChanges()
{
    ...
    if (USourceControlHelpers::IsEnabled())
    {
        ...
        {
            FilePaths.Add(EOSConfiguration::GetFilePath(FM.Get<0>(), FM.Get<1>()));
        }
        ...
        if (!USourceControlHelpers::CheckOutFiles(FilePathsForCheckout.Array(), true))
        {
            ...
            EOSSendCustomEditorSignal(TEXT("Configuration"), TEXT("SourceControlPreventingConfigSave"));
            return FEOSConfigurationWriter::EFlushChangesResult::FailedToCheckOutFiles;
        }
    }
    ...
    if (this->bRequireFullReload)
    {
        ...
        for (const auto &KV : this->FilesQueuedForModify)
        {
            ...
            case EEOSConfigurationFileType::TrustedClient:
                break;
            default:
                checkf(false, TEXT("FEOSConfigurationWriter handling unsupported EEOSConfigurationFileType"));
            }
        }
    ...
    return EFlushChangesResult::Success;
}