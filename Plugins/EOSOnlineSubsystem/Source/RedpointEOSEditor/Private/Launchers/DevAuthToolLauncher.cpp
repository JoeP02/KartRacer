...
FTSTicker::FDelegateHandle FDevAuthToolLauncher::ResetCheckHandle;
...
#if PLATFORM_WINDOWS
    UE_LOG(LogRedpointEOSEditor, Verbose, TEXT("Checking if EOS_DevAuthTool.exe process is running"));
    bIsDevToolRunning = FPlatformProcess::IsApplicationRunning(TEXT("EOS_DevAuthTool.exe"));
#else
    UE_LOG(LogRedpointEOSEditor, Verbose, TEXT("Checking if EOS_DevAuthTool process is running"));
...
bool FDevAuthToolLauncher::Launch(bool bInteractive)
{
    ...
    if (!Path.IsEmpty())
    {
        ...
        )
        {
            ...
            if (FoundZip.IsEmpty())
            {
                UE_LOG(LogRedpointEOSEditor, Error, TEXT("Have path to ZIP, but ZIP doesn't exist"));
                return false;
            }
            ...
            UE_LOG(LogRedpointEOSEditor, Verbose, TEXT("Executing: %s"), *ExtractionBat);
            UE_LOG(LogRedpointEOSEditor, Verbose, TEXT("With arguments: %s"), *Path);
            UE_LOG(LogRedpointEOSEditor, Verbose, TEXT("In working directory: %s"), *ExtractionDir);
...
#endif
            UE_LOG(LogRedpointEOSEditor, Verbose, TEXT("ZIP extraction return code: %d"), ReturnCode);
            UE_LOG(LogRedpointEOSEditor, Verbose, TEXT("ZIP extraction stdout: %s"), *Stdout);
            UE_LOG(LogRedpointEOSEditor, Verbose, TEXT("ZIP extraction stderr: %s"), *Stderr);
            ...
            if (ReturnCode != 0)
            {
                UE_LOG(LogRedpointEOSEditor, Error, TEXT("ZIP extraction exited with non-zero exit code!"));
                return false;
            }
#endif
        }
