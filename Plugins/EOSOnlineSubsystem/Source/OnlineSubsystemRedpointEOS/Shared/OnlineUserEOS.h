...
public:
    ...
    FEOSUserFactory(
        ...
        TSharedRef<Redpoint::EOS::Core::IRuntimePlatform> InRuntimePlatform)
...
class ONLINESUBSYSTEMREDPOINTEOS_API FEOSUserHelpers
{
protected:
    void LogInvalidExternalAccountProvider(const FString &ExternalAccountType) const;
    void LogMissingExternalAccountDataForPlatform(const FString &ExternalAccountType) const;
    void LogUsageOfDeprecatedEpicAccountId() const;
    void LogUsageOfDeprecatedEpicCountry() const;
};

template <typename TDerivedClass, typename TBaseClass>
class TBaseUserImplementation : public TBaseClass,
                                public TSharedFromThis<TDerivedClass>,
                                public IEOSUser,
                                public FEOSUserHelpers
{
private:
...
public:
    ...
    bool GetUserAttribute(const FString &AttrName, FString &OutAttrValue) const override
    {
        ...
        else if (AttrName.StartsWith(TEXT("externalAccount.")) && this->IsReady())
        {
            ...
            {
                FEOSUserHelpers::LogInvalidExternalAccountProvider(ExternalAccountType);
                return false;
            }

            if (!this->GetData().ExternalInfo.Contains(ExternalType))
            {
                FEOSUserHelpers::LogMissingExternalAccountDataForPlatform(ExternalAccountType);
                return false;
            }
        ...
        {
            FEOSUserHelpers::LogUsageOfDeprecatedEpicAccountId();
            return GetUserAttribute(TEXT("externalAccount.epic.id"), OutAttrValue);
        }
        else if (AttrName == "country")
        {
            FEOSUserHelpers::LogUsageOfDeprecatedEpicCountry();
...
public:
    ...
    virtual const TMap<FString, FString> &GetAuthAttributes() const
    {
