...
private:
    ...
    TSharedPtr<EOSEventHandle<EOS_AntiCheatClient_OnClientIntegrityViolatedCallbackInfo>> NotifyClientIntegrityViolated;
#endif

    void EnsureSessionIsCurrent(FAntiCheatSession &Session, const TCHAR *FunctionName) const;

public:
    FEOSGameAntiCheat(EOS_HPlatform InPlatform, bool bInHasTrustedClientPrivateKey);
