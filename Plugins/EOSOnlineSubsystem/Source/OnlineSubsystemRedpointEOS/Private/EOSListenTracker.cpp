    ...
    const TArray<TSharedPtr<FInternetAddr>> &InDeveloperInternetAddrs)
{
    ...
    if (Arr.Num() == 1)
    {
        ...
        if (EOSString_ProductUserId::IsValid(InProductUserId) &&
            EOSString_ProductUserId::ToString(InProductUserId, ProductUserIdString) == EOS_EResult::EOS_Success)
        {
            UE_LOG(
                LogRedpointEOSSessionListening,
                Verbose,
                TEXT("User ID %s is now listening on one or more internet addresses."),
                *ProductUserIdString);
        }
        ...
        if (Arr.Num() == 0)
        {
                ...
                EOSString_ProductUserId::ToString(InProductUserId, ProductUserIdString) == EOS_EResult::EOS_Success)
            {
                UE_LOG(
                    LogRedpointEOSSessionListening,
                    Verbose,
                    TEXT("User ID %s is no longer listening on any internet addresses."),
                    *ProductUserIdString);
            }
        ...
        else
        {
                ...
                EOSString_ProductUserId::ToString(InProductUserId, ProductUserIdString) == EOS_EResult::EOS_Success)
            {
                UE_LOG(
                    LogRedpointEOSSessionListening,
                    Verbose,
                    TEXT("User ID %s is now listening on a different internet address."),
                    *ProductUserIdString);
            }
