...
bool FEOSSocketRoleListening::Close(FSocketEOSFull &Socket) const
{
    ...
    if (!Socket.BindAddress.IsValid())
    {
        UE_LOG(
            LogRedpointEOS,
            Warning,
            TEXT("%d: Attempted to call FEOSSocketRoleListening::Close but the socket has not been locally bound."),
...
bool FEOSSocketRoleListening::Listen(FSocketEOSFull &Socket, int32 MaxBacklog) const
{
    ...
    if (!Socket.BindAddress.IsValid())
    {
        UE_LOG(
            LogRedpointEOS,
            Error,
            TEXT("%d: Attempted to call FEOSSocketRoleListening::Listen but the socket has not been locally bound."),
            Socket.SocketMemoryId);
        return false;
    }
    ...
    const FInternetAddrEOSFull &InRemoteAddr) const
{
    ...
    if (!Socket.BindAddress.IsValid())
    {
        UE_LOG(
            LogRedpointEOS,
            Error,
            TEXT("%d: Attempted to call FEOSSocketRoleListening::Accept but the socket has not been locally bound."),
            Socket.SocketMemoryId);
        return false;
    }
    ...
    const FInternetAddrEOSFull &Destination) const
{
    ...
    UE_LOG(LogRedpointEOS, Error, TEXT("SentTo on a listening socket didn't match any connected client!"));
    return false;
}
...
{
    UE_LOG(LogRedpointEOS, Error, TEXT("GetPeerAddress called on a listening socket!"));
    return false;
}
