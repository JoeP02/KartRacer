...
bool FEOSSocketRoleNone::Listen(FSocketEOSFull &Socket, int32 MaxBacklog) const
{
    ...
    UE_LOG(LogRedpointEOSSocketLifecycle, Verbose, TEXT("%d: Socket moved to: RoleInstance_Listening"), Socket.SocketMemoryId);
...
bool FEOSSocketRoleNone::Connect(FSocketEOSFull &Socket, const FInternetAddrEOSFull &InDestAddr) const
{
    ...
    UE_LOG(LogRedpointEOSSocketLifecycle, Verbose, TEXT("%d: Socket moved to: RoleInstance_Client"), Socket.SocketMemoryId);
...
{
    UE_LOG(LogRedpointEOS, Error, TEXT("GetPeerAddress called for socket with the None role."));
    return false;
}
