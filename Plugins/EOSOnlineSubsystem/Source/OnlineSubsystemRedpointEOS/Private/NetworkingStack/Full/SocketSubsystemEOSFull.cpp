...
FSocketEOSMemoryId FSocketSubsystemEOSFull::NextSocketId = 1000;
...
bool FSocketSubsystemEOSFull::Init(FString &Error)
{
    ...
    this->ReceivePacketsDelegateHandle = Redpoint::EOS::Core::Utils::FRegulatedTicker::GetCoreTicker().AddTicker(
...
void FSocketSubsystemEOSFull::Shutdown()
{
    ...
    Redpoint::EOS::Core::Utils::FRegulatedTicker::GetCoreTicker().RemoveTicker(this->ReceivePacketsDelegateHandle);
...
bool FSocketSubsystemEOSFull::Tick_ReceiveP2PPackets(float DeltaSeconds)
{
    ...
    for (const auto &UserAccount : UserAccounts)
    {
        ...
        while (true)
        {
            ...
            TSharedPtr<FEOSPacketTiming> TimingData;
            ...
            if (CVarEOSEnablePacketTiming.GetValueOnGameThread())
            {
                if (BytesRead > sizeof(int64))
                {
                    ...
                    TimingData = MakeShared<FEOSPacketTiming>(
                        SourceUserId,
                        DestinationUserId,
                        SymmetricSocketId,
                        SymmetricChannel,
            ...
            else
            {
                ...
                if (this->AssignedSockets.Contains(SocketKey))
                {
                    if (auto Socket = this->AssignedSockets[SocketKey].Pin())
                    {
                        ...
                        Socket->ReceivedPacketsQueue.Enqueue(MakeShared<FEOSRawPacket>(Buffer, BytesRead, TimingData));
                    }
                    else
                    {
                        UE_LOG(
                ...
                else
                {
                    ...
                    if (this->ListenManager->GetListeningSocketForNewInboundConnection(SocketKey, ListeningSocket))
                    {
                        ...
                        AcceptedSocket->ReceivedPacketsQueue.Enqueue(
                            MakeShared<FEOSRawPacket>(Buffer, BytesRead, TimingData));
                    }
                    else
                    {
...
{
    FSocketEOSMemoryId SocketId = FSocketSubsystemEOSFull::NextSocketId++;
