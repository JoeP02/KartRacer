...
{
    UE_LOG(LogRedpointEOS, Error, TEXT("FInternetAddrEOSFull::SetIp is not supported."));
}
...
void FInternetAddrEOSFull::SetIp(const TCHAR *InAddr, bool &bIsValid)
{
    ...
    {
        bIsValid = false;
        UE_LOG(LogRedpointEOS, Error, TEXT("FInternetAddrEOSFull::SetIp does not support plain IP addresses yet."));
        return;
    }
    ...
    {
        bIsValid = false;
        UE_LOG(LogRedpointEOS, Error, TEXT("FInternetAddrEOSFull::SetIp received an incomplete address: '%s'"), *Addr);
        return;
    }
    ...
    {
        bIsValid = false;
        UE_LOG(LogRedpointEOS, Error, TEXT("FInternetAddrEOSFull::SetIp did not have a valid socket name."));
        return;
    }
...
{
    UE_LOG(LogRedpointEOS, Error, TEXT("FInternetAddrEOSFull::GetIp is not supported and will set OutAddr to 0."));
    OutAddr = 0;
}
...
void FInternetAddrEOSFull::SetRawIp(const TArray<uint8> &RawAddr)
{
    ...
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("FInternetAddrEOSFull::SetRawIp did not have a valid socket name."));
        return;
    }
...
TArray<uint8> FInternetAddrEOSFull::GetRawIp() const
{
    ...
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("FInternetAddrEOSFull::GetRawIp could not serialize the current user ID."));
        return TArray<uint8>();
    }
...
{
    UE_LOG(LogRedpointEOS, Error, TEXT("FInternetAddrEOSFull::SetAnyAddress is not supported."));
}

void FInternetAddrEOSFull::SetBroadcastAddress()
{
    UE_LOG(LogRedpointEOS, Error, TEXT("FInternetAddrEOSFull::SetBroadcastAddress is not supported."));
}

void FInternetAddrEOSFull::SetLoopbackAddress()
{
    UE_LOG(LogRedpointEOS, Error, TEXT("FInternetAddrEOSFull::SetLoopbackAddress is not supported."));
}
...
FString FInternetAddrEOSFull::ToString(bool bAppendPort) const
{
    ...
    if (EOSString_ProductUserId::ToString(this->UserId, ProductUserIdStr) != EOS_EResult::EOS_Success)
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("FInternetAddrEOSFull::ToString could not serialize the current user ID."));
        return TEXT("");
    }
...
FORCEINLINE uint32 FInternetAddrEOSFullGetTypeHashForString(const FString &InStr)
{
    return GetTypeHash(InStr);
}

uint32 FInternetAddrEOSFull::GetTypeHash() const
{
    return FInternetAddrEOSFullGetTypeHashForString(this->ToString(true));
}
