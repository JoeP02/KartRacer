...
bool FEOSSocketRoleClient::Connect(FSocketEOSFull &Socket, const FInternetAddrEOSFull &InDestAddr) const
{
    ...
    if (!Socket.BindAddress.IsValid())
    {
        UE_LOG(
            LogRedpointEOS,
            Error,
            TEXT("%d: Attempted to call FEOSSocketRoleClient::Connect but the socket has not been locally bound."),
    ...
    const FInternetAddrEOSFull &Destination) const
{
    ...
    if (!Socket.BindAddress.IsValid())
    {
        UE_LOG(
            LogRedpointEOS,
            Error,
            TEXT("%d: Attempted to call FEOSSocketRoleClient::SendTo but the socket has not been locally bound."),
            Socket.SocketMemoryId);
        return false;
    }
...
#if EOS_VERSION_AT_LEAST(1, 15, 3)
        Opts.bDisableAutoAcceptConnection = false;
#endif
