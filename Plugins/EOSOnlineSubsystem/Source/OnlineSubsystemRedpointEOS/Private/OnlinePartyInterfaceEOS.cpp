...
void FOnlinePartySystemEOS::Handle_JoinLobbyAccepted(const EOS_Lobby_JoinLobbyAcceptedCallbackInfo *Data)
{
    ...
    UE_LOG(LogRedpointEOS, Verbose, TEXT("Received JoinLobbyAccepted event from EOS Lobbies system"));
...
void FOnlinePartySystemEOS::Handle_LobbyInviteAccepted(const EOS_Lobby_LobbyInviteAcceptedCallbackInfo *Data)
{
    ...
    if (EOS_Lobby_CopyLobbyDetailsHandleByInviteId(this->EOSLobby, &Opts, &Handle) != EOS_EResult::EOS_Success)
    {
        UE_LOG(
            LogRedpointEOS,
            Error,
            TEXT("Failed to retrieve lobby details from invite ID during LobbyInviteReceived"));
        return;
    }
    ...
    this->JoinParty(
        ...
        FOnJoinPartyComplete::CreateLambda([WeakThis = GetWeakThis(this), LocalUserEOS, JoinInfo, TargetUserEOS](
            ...
            if (auto This = PinWeakThis(WeakThis))
            {
                if (Result != EJoinPartyCompletionResult::Succeeded)
                {
                    UE_LOG(LogRedpointEOS, Error, TEXT("Failed to join lobby that was accepted through the UI"));
                    return;
                }
...
void FOnlinePartySystemEOS::Handle_LobbyUpdateReceived(const EOS_Lobby_LobbyUpdateReceivedCallbackInfo *Data)
{
    UE_LOG(
        LogRedpointEOS,
        Verbose,
        TEXT("PartyInterface: LobbyUpdateReceived: (Lobby) %s"),
            ...
            if (auto This = PinWeakThis(WeakThis))
            {
                ...
                for (uint32_t i = 0; i < Count; i++)
                {
                    ...
                    if (EOS_Lobby_GetInviteIdByIndex(This->EOSLobby, &GetOpts, Buffer, &BufferLen) !=
                        EOS_EResult::EOS_Success)
                    {
                        UE_LOG(LogRedpointEOS, Warning, TEXT("Unable to get invite ID by index during RestoreInvites"));
                                    ...
                                    auto Finalize = [WeakThis = GetWeakThis(This),
                                        ...
                                        if (auto This = PinWeakThis(WeakThis))
                                        {
                                            ...
                                            UE_LOG(
                                                LogRedpointEOS,
                                                Verbose,
                                                TEXT("CreateParty: Added party to parties list"));
                                            ...
                                            This->VoiceManager->UserManuallyJoinedLobby(
                                                StaticCastSharedRef<const FUniqueNetIdEOS>(LocalUserIdRef).Get(),
                                                PartyId.Get());
    ...
    const FOnJoinPartyComplete &Delegate)
{
    ...
    if (OnlinePartyJoinInfoEOSBase->IsUnresolved())
    {
        ...
        // with the resolved data.
        {
            ...
            {
                UE_LOG(LogRedpointEOS, Error, TEXT("Unable to create search for looking up party by ID"));
                Delegate.ExecuteIfBound(
                    LocalUserId,
                    *OnlinePartyJoinInfo.GetPartyId(),
                    EJoinPartyCompletionResult::UnknownClientFailure,
            ...
            if (SearchSetIdResult != EOS_EResult::EOS_Success)
            {
                ...
                UE_LOG(
                    LogRedpointEOS,
                    Error,
                    TEXT("Unable to execute EOS_LobbySearch_SetLobbyId for looking up party by ID"));
                Delegate.ExecuteIfBound(
                    LocalUserId,
                    *OnlinePartyJoinInfo.GetPartyId(),
                    EJoinPartyCompletionResult::UnknownClientFailure,
    ...
    CreateJoinMutex.Run([WeakThis = GetWeakThis(this),
        ...
        if (auto This = PinWeakThis(WeakThis))
        {
            ...
            {
                UE_LOG(LogRedpointEOS, Error, TEXT("JoinParty: EOSJoinInfo is not valid"));
                Delegate.ExecuteIfBound(
                    LocalUserIdRef.Get(),
            ...
            if (LocalUserIdRef.Get().GetType() != REDPOINT_EOS_SUBSYSTEM)
            {
                UE_LOG(LogRedpointEOS, Error, TEXT("JoinParty: Local user ID type is not EOS"));
                Delegate.ExecuteIfBound(
                    LocalUserIdRef.Get(),
                    OnlinePartyJoinInfoRef.Get().GetPartyId().Get(),
                    EJoinPartyCompletionResult::UnknownClientFailure,
            ...
            if (!EOSUser->HasValidProductUserId())
            {
                UE_LOG(LogRedpointEOS, Error, TEXT("JoinParty: Local user does not have a valid product user ID"));
                Delegate.ExecuteIfBound(
                    LocalUserIdRef.Get(),
                    OnlinePartyJoinInfoRef.Get().GetPartyId().Get(),
                    EJoinPartyCompletionResult::UnknownClientFailure,
                            ...
                            if (auto This = PinWeakThis(WeakThis))
                            {
                                ...
                                // Just try to get the RTC room name; if it succeeds then this lobby is RTC enabled.
                                {
                                    ...
                                    if (VoiceUser.IsValid())
                                    {
                                        ...
                                        if (GetRoomNameResult == EOS_EResult::EOS_Success)
                                        {
                                            ...
                                            if (bEnableEcho)
                                            {
                                                This->VoiceManager->MarkPartyOrLobbyAsEchoEnabled(EOSUser, EOSPartyId);
                                            }
                                        }
                                    }
                                }
                                ...
                                auto Finalize = [WeakThis = GetWeakThis(This),
                                    ...
                                    if (auto This = PinWeakThis(WeakThis))
                                    {
                                        ...
                                        UE_LOG(LogRedpointEOS, Verbose, TEXT("JoinParty: Added party to parties list"));
                                        if (!This->JoinedParties.Contains(*LocalUserIdRef))
                                        {
                                            This->JoinedParties.Add(
                                                *LocalUserIdRef,
                                        ...
                                        UE_LOG(LogRedpointEOS, Verbose, TEXT("JoinParty: Succeeded"));
                                        Delegate.ExecuteIfBound(
                                            LocalUserIdRef.Get(),
                                            EOSPartyId.Get(),
            ...
            This->LookupPartyById(
                ...
                [Delegate, LocalUserIdRef, EOSPartyId, MutexRelease, LeaveLobbyWithPresenceIfNecessary](
                    ...
                    {
                        UE_LOG(LogRedpointEOS, Error, TEXT("JoinParty: Failed to resolve party ID to lobby handle"));
                        Delegate.ExecuteIfBound(
                            LocalUserIdRef.Get(),
                            EOSPartyId.Get(),
                            EJoinPartyCompletionResult::UnknownClientFailure,
...
{
    UE_LOG(LogRedpointEOS, Error, TEXT("JIPFromWithinParty not supported"));
    return false;
}
#endif
...
{
    UE_LOG(LogRedpointEOS, Error, TEXT("QueryPartyJoinability not supported"));
    Delegate.ExecuteIfBound(
        LocalUserId,
...
bool FOnlinePartySystemEOS::RejoinParty(
    ...
    const FOnJoinPartyComplete &Delegate)
{
    UE_LOG(LogRedpointEOS, Error, TEXT("RejoinParty not supported"));
    return false;
}
    ...
    const FOnLeavePartyComplete &Delegate)
{
    ...
    if (LocalUserId.GetType() != REDPOINT_EOS_SUBSYSTEM)
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("DoLeaveParty: Local user ID type is not EOS"));
    ...
    if (!EOSUser->HasValidProductUserId())
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("DoLeaveParty: Local EOS user does not have a valid product user ID"));
        Delegate.ExecuteIfBound(LocalUserId, PartyId, ELeavePartyCompletionResult::UnknownClientFailure);
        return true;
    }
    ...
    EOSRunOperation<EOS_HLobby, EOS_Lobby_LeaveLobbyOptions, EOS_Lobby_LeaveLobbyCallbackInfo>(
        ...
        &Opts,
        *EOS_Lobby_LeaveLobby,
        [WeakThis = GetWeakThis(this), Delegate, LocalUserIdRef, EOSUser, EOSPartyId](
            const EOS_Lobby_LeaveLobbyCallbackInfo *Info) {
            if (auto This = PinWeakThis(WeakThis))
            {
                if (Info->ResultCode != EOS_EResult::EOS_Success)
                ...
                This->VoiceManager->UserManuallyLeftLobby(*EOSUser, *EOSPartyId);

                UE_LOG(LogRedpointEOS, Verbose, TEXT("DoLeaveParty: Party left successfully"));
...
{
    UE_LOG(LogRedpointEOS, Error, TEXT("ApproveJoinRequest not supported!"));
    return false;
}
...
bool FOnlinePartySystemEOS::ApproveJIPRequest(
    ...
    int32 DeniedResultCode)
{
    UE_LOG(LogRedpointEOS, Error, TEXT("ApproveJIPRequest not supported!"));
    return false;
}
#endif
...
void FOnlinePartySystemEOS::RespondToQueryJoinability(
    ...
    int32 DeniedResultCode)
{
    UE_LOG(LogRedpointEOS, Error, TEXT("RespondToQueryJoinability not supported!"));
}
#endif
...
{
    UE_LOG(LogRedpointEOS, Error, TEXT("CancelInvitation not supported!"));
...
bool FOnlinePartySystemEOS::RejectInvitation(const FUniqueNetId &LocalUserId, const FUniqueNetId &SenderId)
{
    ...
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("RejectInvitation: Local user ID or sender user ID were not EOS user IDs"));
        return false;
    }
    ...
    if (!EOSUser->HasValidProductUserId())
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("RejectInvitation: Local user ID did not have product user ID"));
        return false;
    }
    ...
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("RejectInvitation: Sender user ID did not have product user ID"));
        return false;
    }
    ...
    const FOnRejectPartyInvitationComplete &Delegate)
{
    ...
    if (LocalUserId.GetType() != REDPOINT_EOS_SUBSYSTEM)
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("RejectInvitation: Local user ID was not an EOS user ID"));
        return false;
    }
    ...
    if (EOSJoinInfo == nullptr)
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("RejectInvitation: Join info is not valid"));
        return false;
    }
...
void FOnlinePartySystemEOS::ClearInvitations(
    ...
    const FOnlinePartyId *PartyId)
{
    UE_LOG(LogRedpointEOS, Error, TEXT("ClearInvitations not supported!"));
}
        ...
        UE_LOG(
            ...
            *OnlineRedpointEOS::Errors::InvalidRequest(
                 ...
                 *PartyId.ToString(),
                 ...
                 TEXT(
                     "EOS does not support namespaced party data. The namespace parameter will be ignored. You should "
                     "use "
                ...
                else if (Info->ResultCode == EOS_EResult::EOS_NoChange)
                {
                    UE_LOG(
                        ...
                        *ConvertError(
                             ...
                             *PartyIdRef->ToString(),
                             ANSI_TO_TCHAR(__FUNCTION__),
                             TEXT("The UpdatePartyMemberData operation did not change any data on the party member, "
                                  "which means the OnPartyMemberDataReceived event will not be fired."),
                             Info->ResultCode)
                             .ToLogString());
                }
    ...
    const FUniqueNetId &MemberId) const
{
    ...
    UE_LOG(LogRedpointEOS, Error, TEXT("IsMemberLeader called for unknown party!"));
    return false;
}
...
uint32 FOnlinePartySystemEOS::GetPartyMemberCount(const FUniqueNetId &LocalUserId, const FOnlinePartyId &PartyId) const
{
    ...
    UE_LOG(LogRedpointEOS, Error, TEXT("GetPartyMemberCount called for unknown party!"));
    return 0;
}
    ...
    const
{
    ...
    UE_LOG(LogRedpointEOS, Error, TEXT("GetParty called for unknown party!"));
    return nullptr;
}
    ...
    const FUniqueNetId &MemberId) const
{
    ...
    for (const auto &Party : this->JoinedParties[LocalUserId])
    {
        if (Party->PartyId.Get() == PartyId)
        {
            ...
            UE_LOG(LogRedpointEOS, Error, TEXT("GetPartyMember called for unknown party member!"));
            return nullptr;
        }
    }

    UE_LOG(LogRedpointEOS, Error, TEXT("GetPartyMember called for unknown party!"));
    return nullptr;
}
    ...
    const FName &Namespace) const
{
    ...
    UE_LOG(LogRedpointEOS, Error, TEXT("GetPartyData called for unknown party!"));
    return nullptr;
}
    ...
    const FName &Namespace) const
{
    ...
    for (const auto &Party : this->JoinedParties[LocalUserId])
    {
        if (Party->PartyId.Get() == PartyId)
        {
            ...
            UE_LOG(LogRedpointEOS, Error, TEXT("GetPartyMemberData called for unknown party member!"));
            return nullptr;
        }
    }

    UE_LOG(LogRedpointEOS, Error, TEXT("GetPartyMemberData called for unknown party!"));
    return nullptr;
}
...
IOnlinePartyJoinInfoConstPtr FOnlinePartySystemEOS::GetAdvertisedParty(
    ...
    const FOnlinePartyTypeId PartyTypeId) const
{
    UE_LOG(LogRedpointEOS, Error, TEXT("GetAdvertisedParty not supported!"));
    return nullptr;
}
    ...
    TArray<TSharedRef<const FOnlinePartyId>> &OutPartyIdArray) const
{
    if (!this->JoinedParties.Contains(LocalUserId))
    {
        UE_LOG(LogRedpointEOS, VeryVerbose, TEXT("GetJoinedParties: There are 0 parties present"));
    ...
    TArray<FOnlinePartyMemberConstRef> &OutPartyMembersArray) const
{
    ...
    UE_LOG(LogRedpointEOS, Error, TEXT("GetPartyMembers called for unknown party!"));
    return false;
}
...
{
    UE_LOG(LogRedpointEOS, Error, TEXT("GetPendingJoinRequests is not supported!"));
    return false;
}
...
{
    UE_LOG(LogRedpointEOS, Error, TEXT("GetPendingInvitedUsers is not supported!"));
    return false;
}
...
#if defined(UE_5_2_OR_LATER)
IOnlinePartyJoinInfoConstPtr FOnlinePartySystemEOS::MakeJoinInfo(
    const FUniqueNetId &LocalUserId,
    const FOnlinePartyId &PartyId)
{
    return nullptr;
