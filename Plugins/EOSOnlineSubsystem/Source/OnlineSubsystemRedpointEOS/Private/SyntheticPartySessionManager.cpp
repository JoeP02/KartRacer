...
{
    FString SessionName =
        FString::Printf(TEXT("SyntheticSession_%s_%s"), *InSubsystemName.ToString(), *InSessionId->ToString());
    for (const auto &Integration : this->RuntimePlatform->GetIntegrations())
    {
        Integration->MutateSyntheticSessionSessionNameIfRequired(InSubsystemName, InSessionId->ToString(), SessionName);
    }
    return FName(*SessionName);
}

FName FSyntheticPartySessionManager::GetSyntheticSessionNativePartyName(
    const FName &InSubsystemName,
    const TSharedPtr<const FOnlinePartyId> &InPartyId)
{
    FString SessionName =
        FString::Printf(TEXT("SyntheticParty_%s_%s"), *InSubsystemName.ToString(), *InPartyId->ToString());
    for (const auto &Integration : this->RuntimePlatform->GetIntegrations())
    {
        Integration->MutateSyntheticPartySessionNameIfRequired(InSubsystemName, InPartyId->ToString(), SessionName);
    }
    return FName(*SessionName);
}
            ...
            this->SessionUserInviteAcceptedDelegateHandles.Add(
                ...
                Session->AddOnSessionUserInviteAcceptedDelegate_Handle(
                    FOnSessionUserInviteAcceptedDelegate::CreateLambda([WeakThis = GetWeakThis(this)](
                        ...
                        {
                            UE_LOG(LogRedpointEOS, Verbose, TEXT("Received invite from synthetic party"));
    ...
    if (EOSParties && EOSIdentity)
    {
        ...
        if (!EOSParties->GetJoinedParties(*IdentityByController, JoinedParties))
        {
            UE_LOG(
                LogRedpointEOS,
                Error,
                TEXT("GetJoinedParties returned error while processing synthetic party invite"));
            return;
        }
            ...
            [EOSParties, IdentityByController](EOS_HLobbyDetails LobbyHandle) {
                ...
                EOSParties->JoinParty(
                    ...
                    FOnJoinPartyComplete::CreateLambda([](const FUniqueNetId &LocalUserId,
                        ...
                        UE_LOG(LogRedpointEOS, Verbose, TEXT("Join party from synthetic party: %d"), Result);
                    }));
            });
    }
    else
...
void FSyntheticPartySessionManager::HandleSessionInviteAccept(int32 ControllerId, const FString &EOSSessionId)
{
    ...
    {
        if (EOSIdentity->GetLoginStatus(ControllerId) == ELoginStatus::NotLoggedIn)
        {
            // Routing of accepted invites in the editor is a bit jank, since wrapped subsystems might be singletons. If
            // they are, they'll cause both the PIE and global DefaultInstance to receive the event, and in that case
            // you'll get the message below forever (since DefaultInstance doesn't get logged into). In the editor, just
            ...
            if (auto This = PinWeakThis(WeakThis))
            {
                if (auto Session = Subsystem.Session.Pin())
                {
                    if (auto Identity = Subsystem.Identity.Pin())
                    {
                        ...
                        {
                            auto SessionName =
                                This->GetSyntheticSessionNativePartyName(Subsystem.SubsystemName, PartyId);
                            auto CreateHandleName = FString::Printf(TEXT("%s_Create"), *SessionName.ToString());
                            ...
                            for (const auto &Integration : This->RuntimePlatform->GetIntegrations())
                            {
                                Integration->MutateSyntheticPartySessionSettingsIfRequired(
                                ...
                                Session->AddOnCreateSessionCompleteDelegate_Handle(
                                    FOnCreateSessionCompleteDelegate::CreateLambda(
                                        [SessionName, Finalize](FName CreatedSessionName, bool bWasSuccessful) {
                                            if (CreatedSessionName.IsEqual(SessionName))
                                            {
                                                if (!bWasSuccessful)
                                                {
                                                    UE_LOG(
                            ...
                            if (!Session->CreateSession(UserId.ToSharedRef().Get(), SessionName, SessionSettings))
                            {
                                UE_LOG(
                                    LogRedpointEOS,
                                    Error,
...
bool FSyntheticPartySessionManager::HasSyntheticParty(const TSharedPtr<const FOnlinePartyId> &PartyId)
{
    ...
    FMultiOperation<FWrappedSubsystem, bool>::Run(
        this->WrappedSubsystems,
        [WeakThis = GetWeakThis(this), PartyId, EOSPartyId, &bHasSyntheticParty](
            const FWrappedSubsystem &Subsystem,
            ...
            if (auto This = PinWeakThis(WeakThis))
            {
                if (auto Session = Subsystem.Session.Pin())
                {
                    if (auto Identity = Subsystem.Identity.Pin())
                    {
                        ...
                        {
                            auto SessionName =
                                This->GetSyntheticSessionNativeSessionName(Subsystem.SubsystemName, SessionId);
                            auto CreateHandleName = FString::Printf(TEXT("%s_Create"), *SessionName.ToString());
                            ...
                            for (const auto &Integration : This->RuntimePlatform->GetIntegrations())
                            {
                                Integration->MutateSyntheticSessionSessionSettingsIfRequired(
                                    Subsystem.SubsystemName,
                                    SessionId->ToString(),
                                    SessionSettings);
                            }
    ...
    for (const auto &Subsystem : this->WrappedSubsystems)
    {
        if (Subsystem.SubsystemName.IsEqual(SubsystemName))
        {
            if (auto Session = Subsystem.Session.Pin())
            {
                if (auto Identity = Subsystem.Identity.Pin())
                {
                    auto SessionName = this->GetSyntheticSessionNativeSessionName(Subsystem.SubsystemName, SessionId);
    ...
    const FSyntheticPartySessionOnComplete &Delegate)
{
    ...
    if (auto IdentityEOS = PinWeakThis(this->EOSIdentitySystem))
    {
        for (const auto &Subsystem : this->WrappedSubsystems)
        {
            ...
            auto SessionName = this->GetSyntheticSessionNativePartyName(Subsystem.SubsystemName, PartyId);
            ...
            if (auto SessionInterface = Subsystem.Session.Pin())
            {
                         ...
                         ->SendSessionInviteToFriend(LocalUserNum, SessionName, RecipientId.ToSharedRef().Get()))
                {
                    ...
                    UE_LOG(LogRedpointEOS, Error, TEXT("%s"), *CallUnexpectedlyFailedError.ToLogString());
            ...
            auto SessionInterfaceNoLongerAvailableError = OnlineRedpointEOS::Errors::Canceled(
                ...
                FString::Printf(
                    ...
                    *Subsystem.SubsystemName.ToString()));
            UE_LOG(LogRedpointEOS, Error, TEXT("%s"), *SessionInterfaceNoLongerAvailableError.ToLogString());
        ...
        UE_LOG(LogRedpointEOS, Error, TEXT("%s"), *InvalidRecipientError.ToLogString());
