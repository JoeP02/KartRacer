    ...
    FOnGetAvatarComplete OnComplete)
{
    ...
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("LocalUserId is not an EOS user when calling GetAvatar."));
        OnComplete.ExecuteIfBound(false, DefaultTexture);
        return true;
    }
    ...
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("LocalUserId is not a locally signed in EOS user."));
        OnComplete.ExecuteIfBound(false, DefaultTexture);
        return true;
    }
    ...
    FOnGetAvatarUrlComplete OnComplete)
{
    ...
    if (LocalUserId.GetType() != REDPOINT_EOS_SUBSYSTEM)
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("LocalUserId is not an EOS user when calling GetAvatarUrl."));
