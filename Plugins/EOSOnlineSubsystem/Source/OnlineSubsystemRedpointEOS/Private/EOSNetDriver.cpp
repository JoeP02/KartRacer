    ...
    TSharedPtr<class FOnlineSubsystemEOS, ESPMode::ThreadSafe> &OutOnlineSubsystem)
{
    ...
    if (WorldRef == nullptr)
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("UEOSNetDriver could not locate the current world."));
        return false;
    }
    ...
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("No online subsystem is present, so UEOSNetDriver can not work."));
        return false;
    }
    if (Subsystem->GetSubsystemName() != REDPOINT_EOS_SUBSYSTEM)
    {
        UE_LOG(
            LogRedpointEOS,
            Error,
            TEXT("The current online subsystem is not the EOS online subsystem, so UEOSNetDriver can not work. The "
                 "current subsystem is '%s'."),
    ...
    if (RedpointEOSModule != nullptr)
    {
        if (((FOnlineSubsystemRedpointEOSModule *)RedpointEOSModule)->IsOperatingInNullMode())
        {
            UE_LOG(
                LogRedpointEOS,
                Error,
                TEXT("The EOS online subsystem was not configured properly when the editor started up, so "
                     "UEOSNetDriver can not work."));
            return false;
        }
    }
#endif
...
void UEOSNetDriver::RequestConnectionClose(UEOSNetConnection *InConnection)
{
...
bool UEOSNetDriver::InitConnect(FNetworkNotify *InNotify, const FURL &InConnectURL, FString &Error)
{
    ...
    if (!this->IsKnownChannelName(NAME_Control))
    {
        UE_LOG(
            LogRedpointEOS,
            Error,
        ...
        if (!this->bIsOwnedByBeacon)
        {
            if (auto AC = this->AntiCheat.Pin())
            {
                ...
                if (OnlineSubsystem->GetIdentityInterface()->GetLoginStatus(0) == ELoginStatus::LoggedIn)
                {
                    ...
                    {
                        UE_LOG(LogRedpointEOS, Error, TEXT("Net driver failed to set up Anti-Cheat session."));
                        return false;
                    }
                    checkf(
    ...
    if (!this->CanPerformP2PNetworking(RefSocketSubsystem))
    {
        UE_LOG(
            LogRedpointEOS,
            Warning,
            TEXT("UEOSNetDriver: Attempting to connect to a P2P address, but CanPerformP2PNetworking reports "
    ...
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("UEOSNetDriver: InitConnect failed while setting up base information"));
        return false;
    }
    ...
    UE_LOG(LogRedpointEOS, Verbose, TEXT("UEOSNetDriver: InitConnect completed successfully"));
    return true;
}
...
bool UEOSNetDriver::InitListen(FNetworkNotify *InNotify, FURL &ListenURL, bool bReuseAddressAndPort, FString &Error)
{
    ...
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("UEOSNetDriver: InitListen failed while setting up base information"));
        return false;
    }
...
void UEOSNetDriver::TickDispatch(float DeltaTime)
{
    ...
    for (UEOSNetConnection *ConnectionToClose : this->PendingConnectionClose)
    {
        if (IsValid(ConnectionToClose))
        {
    ...
    while (true)
    {
        ...
        if (auto SocketSubsystemPinned = this->SocketSubsystem.Pin())
        {
            ...
            if (TSharedPtr<ISocketEOS> SocketPinned = this->Socket.Pin())
            {
                ...
                {
                    FMemory::Free(Buffer);
                    UE_LOG(LogRedpointEOS, Error, TEXT("UEOSNetDriver::TickDispatch: Socket RecvFrom failed!"));
                    break;
                }
            }
            else
        ...
        {
            FMemory::Free(Buffer);
            UE_LOG(LogRedpointEOS, Error, TEXT("UEOSNetDriver::TickDispatch: Socket RecvFrom read 0 bytes of data!"));
            break;
        }
...
void UEOSNetDriver::TickFlush(float DeltaTime)
{
...
void UEOSNetDriver::LowLevelDestroy()
{
    if (auto AC = this->AntiCheat.Pin())
    {
        if (this->AntiCheatSession.IsValid())
        {
            ...
            {
                UE_LOG(LogRedpointEOS, Warning, TEXT("Net driver failed to destroy Anti-Cheat session!"));
            }
