...
void FQueuedLoginEntry::SendSuccess()
{
    ...
    {
        UE_LOG(LogRedpointEOSNetworkAuth, Error, TEXT("Control channel is no longer valid, not sending NMT_Login packet."));
        return;
    }
...
void FQueuedLoginEntry::SendFailure(const FString &ErrorMessage)
{
    if (!ControlChannel.IsValid())
    {
        UE_LOG(LogRedpointEOSNetworkAuth, Error, TEXT("Control channel is no longer valid, not sending NMT_Failure packet."));
        return;
    }
