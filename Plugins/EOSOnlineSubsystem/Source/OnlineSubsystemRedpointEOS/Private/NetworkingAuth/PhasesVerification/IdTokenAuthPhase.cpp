...
void FIdTokenAuthPhase::Start(const TSharedRef<FAuthVerificationPhaseContext> &Context)
{
    ...
    if (!Context->GetConnection(Connection) || !Context->IsConnectionAsTrustedOnClient(IsTrusted))
    {
    ...
    FNetControlMessage<NMT_EOS_RequestIdToken>::Send(Connection, UserIdRepl);
}
...
void FIdTokenAuthPhase::On_NMT_EOS_RequestIdToken(const TSharedRef<FAuthVerificationPhaseContext> &Context)
{
    ...
    if (!Context->GetRole(Role) || !Context->GetConnection(Connection) ||
        !Context->IsConnectionAsTrustedOnClient(IsTrusted) || !Context->GetOSS(OSS))
    {
        Context->Finish(EAuthPhaseFailureCode::All_ConnectionHasGoneAway);
        return;
    }
    ...
    FNetControlMessage<NMT_EOS_DeliverIdToken>::Send(Connection, TargetUserId, Token);
#else
    ...
    const FString &ClientToken)
{
    ...
    FOnlineSubsystemEOS *OSS;
    bool IsTrusted;
    if (!Context->GetConnection(Connection) || !Context->GetRole(Role) || !Context->GetOSS(OSS) ||
        !Context->IsConnectionAsTrustedOnClient(IsTrusted))
    {
        Context->Finish(EAuthPhaseFailureCode::All_ConnectionHasGoneAway);
        return;
    }
