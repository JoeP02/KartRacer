...
void UEOSControlChannel::On_NMT_Hello(const FOriginalParameters_NMT_Hello &Parameters)
{
    ...
    Context->OnCompleted().AddUObject(this, &UEOSControlChannel::Finalize_NMT_Hello, Parameters);
    ...
    const TArray<uint8> &AntiCheatData)
{
    ...
    if (bIsOwnedByBeacon)
    {
        ...
        UE_LOG(LogRedpointEOS, Error, TEXT("Ignoring Anti-Cheat network message for beacon."));
        return;
    }
    if (!AntiCheat.IsValid() || !AntiCheatSession.IsValid())
    {
        // No anti-cheat session.
        UE_LOG(LogRedpointEOS, Error, TEXT("Received Anti-Cheat network message, but no active Anti-Cheat session."));
        return;
    }
    ...
    if (!AntiCheat->ReceiveNetworkMessage(
            ...
            *SourceUserIdEOS,
            ...
            AntiCheatData.Num()))
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("Anti-Cheat network message was not received at control channel level."));
    }
}
    ...
    EOS_EAntiCheatCommonClientAuthStatus NewAuthStatus)
{
    ...
    else
    {
        ...
        if (this->Connection->RemoteAddr.IsValid() &&
            this->Connection->RemoteAddr->GetProtocolType() == REDPOINT_EOS_SUBSYSTEM)
        {
            auto CastedRemoteAddr = StaticCastSharedPtr<IInternetAddrEOS>(this->Connection->RemoteAddr);
            auto RemotePeerUserId = CastedRemoteAddr->GetUserId();
            auto RemotePeerUniqueNetId = MakeShared<FUniqueNetIdEOS>(RemotePeerUserId);
            auto RemotePeerUniqueNetIdRepl = FUniqueNetIdRepl(*RemotePeerUniqueNetId);
            auto Context = this->GetAuthLoginPhaseContext(RemotePeerUniqueNetIdRepl);
    ...
    bool bIsBeacon)
{
    ...
    if (!IncomingUser.IsValid() || IncomingUser.GetType() != REDPOINT_EOS_SUBSYSTEM)
    {
        ...
        return;
#else
        UE_LOG(LogRedpointEOS, Error, TEXT("Missing user ID during connection."));
    ...
    if (!bIsBeacon)
    {
        ...
        PhaseSetupAndStart = [this, LoginContext, NetworkEntry]() {
            const Redpoint::EOS::Config::IConfig *Config;
            EEOSNetDriverRole Role;
            if (!LoginContext->GetConfig(Config) || !LoginContext->GetRole(Role))
            {
...
void UEOSControlChannel::On_NMT_BeaconJoin(const FString &BeaconType, const FUniqueNetIdRepl &IncomingUser)
{
        ...
        for (const auto &KV : this->bRegisteredForAntiCheat)
        {
            if (KV.Value)
            {
                if (AntiCheat.IsValid() && AntiCheatSession.IsValid())
                {
                    ...
                    {
                        UE_LOG(LogRedpointEOS, Warning, TEXT("Anti-Cheat UnregisterPlayer call failed!"));
                    }
                }
                else
                {
...
#endif // #if defined(UE_5_0_OR_LATER)
    {
        ...
        */
            ...
            FNetworkHelpers::GetRole(this->Connection) == EEOSNetDriverRole::ClientConnectedToDedicatedServer)
        {
            ...
            FNetworkHelpers::GetRole(Connection) == EEOSNetDriverRole::DedicatedServer)
        {
    ...
    case NMT_##Type:                                                                                                   \
        ...
        break;
            switch (MessageType)
            {
                ...
                REDPOINT_EOS_DISCARD_MESSAGE_TYPE(EOS_WriteStat)
