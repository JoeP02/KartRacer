        ...
        if (SocketPinned->bClosed)
        {
...
#if defined(UE_5_0_OR_LATER)
            UEOSNetDriver *EOSNetDriver = Cast<UEOSNetDriver>(this->Driver.Get());
#else
            UEOSNetDriver *EOSNetDriver = Cast<UEOSNetDriver>(this->Driver);
#endif
            if (IsValid(EOSNetDriver))
            {
                UE_LOG(
                    LogNet,
                    Verbose,
                    TEXT("UEOSNetConnection::LowLevelSend: Closing the connection as the underlying socket has been "
                         "closed (usually by the remote host)."));
                EOSNetDriver->RequestConnectionClose(this);
            }
            else
            {
                UE_LOG(
                    LogNet,
                    Warning,
                    TEXT("UEOSNetConnection::LowLevelSend: Unable to inform network driver of connection close because "
