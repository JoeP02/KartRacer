...
void FOnlineIdentityInterfaceEOS::AttachMessagingHub(const TSharedRef<IMessagingHub> &InMessagingHub)
{
    this->MessagingHub = InMessagingHub;
}

void FOnlineIdentityInterfaceEOS::DetachMessagingHub()
{
    this->MessagingHub = nullptr;
}
...
bool FOnlineIdentityInterfaceEOS::Exec(UWorld *InWorld, const TCHAR *Cmd, FOutputDevice &Ar)
{
    ...
    if (FParse::Command(&Cmd, TEXT("SIMULATEEXPIRY"), false))
    {
        ...
        UE_LOG(LogRedpointEOS, Verbose, TEXT("Simulating login expiry for local user num %d..."), LocalUserNum);
        this->HandleLoginExpiry(LocalUserNum);
        return true;
    }
#endif // #if EOS_HAS_AUTHENTICATION
...
void FOnlineIdentityInterfaceEOS::DumpLoggedInUsers()
{
    ...
    for (int i = 0; i < ConnectUsers; i++)
    {
        ...
        {
            UE_LOG(LogRedpointEOS, Verbose, TEXT("DumpLoggedInUsers: Connect user %d: %s"), i, *ConnectUserIdStr);
        }
    }
    for (int i = 0; i < AuthAccounts; i++)
    {
        ...
        {
            UE_LOG(LogRedpointEOS, Verbose, TEXT("DumpLoggedInUsers: Epic account %d: %s"), i, *EpicAccountIdStr);
        }
    }
}
    ...
    const FEOSIdentity_AccountLoaded &OnComplete)
{
    ...
    FUserOnlineAccountEOS::Get(
        ...
        [WeakThis = GetWeakThis(this), OnComplete, InAuthAttributes, UserId](
            ...
            if (!Account->IsValid())
            {
                UE_LOG(LogRedpointEOS, Error, TEXT("EOS_LoadAccountInformation: Unable to load account information"));
    ...
    const TSharedPtr<const FUniqueNetId> &InNewUniqueId)
{
    ...
    UE_LOG(
        LogRedpointEOS,
        Verbose,
        TEXT("Updated unique net ID of ULocalPlayer for local player num %d."),
        InLocalPlayerNum);
    ...
    const TSharedPtr<FUserOnlineAccount> InUserAccount)
{
    ...
    for (const auto &WorldContext : GEngine->GetWorldContexts())
    {
        if (WorldContext.WorldType == EWorldType::PIE && WorldContext.PIEInstance == 0)
        {
            ...
            UE_LOG(LogRedpointEOS, Verbose, TEXT("PIE client login completing because the PIE server is now ready"));
            this->FinishLogin(InState, InUserAccount);
            return false;
        }
    }
    ...
    const TSharedPtr<FUserOnlineAccount> &UserAccount)
{
    ...
    if (this->MessagingHub.IsValid())
    {
        this->MessagingHub->LocalUserSignedIn(*State->ResultUserId.Get());
    }
...
bool FOnlineIdentityInterfaceEOS::Login(int32 LocalUserNum, const FOnlineAccountCredentials &AccountCredentials)
{
    ...
    TSharedPtr<FOnlineIdentityInterfaceEOS, ESPMode::ThreadSafe> This = this->AsShared();
...
#endif // #if EOS_HAS_AUTHENTICATION
                {
                    ...
                    UE_LOG(LogRedpointEOS, Verbose, TEXT("DedicatedServer: Performed Login on dedicated server."));
                    UE_LOG(
                        LogRedpointEOSIdentity,
                        Verbose,
                        TEXT("Local user %d is now signed in as %s"),
        ...
        return true;
                ...
                if (This->LoggedInUsers.Contains(LocalUserNum))
                {
                    ...
                    State->ExistingAuthAttributes =
                        StaticCastSharedPtr<FUserOnlineAccountEOS>(This->LoggedInAccounts[LocalUserNum])
                            ->GetAuthAttributes();
...
bool FOnlineIdentityInterfaceEOS::Logout(int32 LocalUserNum)
{
    if (this->LoggedInUsers.Contains(LocalUserNum))
    {
...
#endif // #if EOS_HAS_AUTHENTICATION
        {
            ...
            this->TriggerOnLoginStatusChangedDelegates(
                ...
                *FUniqueNetIdEOS::InvalidId());
            this->TriggerOnLoginChangedDelegates(LocalUserNum);
            UE_LOG(LogRedpointEOS, Verbose, TEXT("DedicatedServer: Performed Logout on dedicated server."));
            UE_LOG(LogRedpointEOSIdentity, Verbose, TEXT("Local user %d is now signed out"), LocalUserNum);
            return true;
        }
        ...
        EOSRunOperation<EOS_HAuth, EOS_Auth_DeletePersistentAuthOptions, EOS_Auth_DeletePersistentAuthCallbackInfo>(
            ...
            [WeakThis = GetWeakThis(this), LocalUserNum](const EOS_Auth_DeletePersistentAuthCallbackInfo *Data) {
                if (auto This = PinWeakThis(WeakThis))
                {
                    ...
                    if (This->MessagingHub.IsValid())
                    {
                        This->MessagingHub->LocalUserSignedOut(
                            (const FUniqueNetIdEOS &)*This->LoggedInUsers[LocalUserNum]);
                    }
                    This->UpdateUniqueNetIdForPlayerController(LocalUserNum, nullptr);
                    if (This->LoggedInUsers.Contains(LocalUserNum))
                    {
    ...
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("Local user %d is not logged in, can not log them out"), LocalUserNum);
...
TSharedPtr<const FUniqueNetId> FOnlineIdentityInterfaceEOS::GetUniquePlayerId(int32 LocalUserNum) const
{
    ...
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("Invalid user %d passed to GetUniquePlayerId"), LocalUserNum);
        return nullptr;
    }
#endif
}
    ...
    bool bObtainRedpointImplementation) const
{
    if (!this->LoggedInUserNativeSubsystemNames.Contains(LocalUserNum) ||
        this->LoggedInUserNativeSubsystemNames[LocalUserNum].IsNone())
    {
