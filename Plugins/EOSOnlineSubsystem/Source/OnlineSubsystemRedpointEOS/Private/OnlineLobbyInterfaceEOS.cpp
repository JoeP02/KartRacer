...
{
    FEOSLobbyDetailsHandle LobbyDetails;
    ...
    EOS_EResult HandleResult =
        LobbyDetails.WithWrite<EOS_EResult>(this->EOSSubsystem, [&](EOS_HLobbyDetails *WriteTarget) {
            return EOS_Lobby_CopyLobbyDetailsHandle(this->EOSLobby, &HandleOpts, WriteTarget);
        });
...
void FOnlineLobbyInterfaceEOS::DumpLobbyAttributeState(
    ...
    const FEOSLobbyDetailsHandle &InLobbyDetails) const
{
    UE_LOG(LogRedpointEOS, Verbose, TEXT("LobbyInterface: --- Lobby State: %s (Begin) ---"), ANSI_TO_TCHAR(InLobbyId));
    UE_LOG(LogRedpointEOS, Verbose, TEXT("LobbyInterface: Info:"));
    ...
    if (InfoResult == EOS_EResult::EOS_Success)
    {
        ...
        UE_LOG(LogRedpointEOS, Verbose, TEXT("LobbyInterface:   Max Members: %u"), Info->MaxMembers);
        UE_LOG(LogRedpointEOS, Verbose, TEXT("LobbyInterface:   Available Slots: %u"), Info->AvailableSlots);
        switch (Info->PermissionLevel)
        {
        case EOS_ELobbyPermissionLevel::EOS_LPL_INVITEONLY:
            UE_LOG(LogRedpointEOS, Verbose, TEXT("LobbyInterface:   Permission Level: Invite Only"));
            break;
        case EOS_ELobbyPermissionLevel::EOS_LPL_JOINVIAPRESENCE:
            UE_LOG(LogRedpointEOS, Verbose, TEXT("LobbyInterface:   Permission Level: Join Via Presence"));
            break;
        case EOS_ELobbyPermissionLevel::EOS_LPL_PUBLICADVERTISED:
            UE_LOG(LogRedpointEOS, Verbose, TEXT("LobbyInterface:   Permission Level: Public Advertised"));
            break;
        }
        UE_LOG(
            LogRedpointEOS,
    ...
    {
        UE_LOG(LogRedpointEOS, Warning, TEXT("LobbyInterface:   Error: %s"), *ConvertError(InfoResult).ToLogString());
    }

    UE_LOG(LogRedpointEOS, Verbose, TEXT("LobbyInterface: Attributes:"));

    {
        ...
        for (uint32_t AttrIdx = 0; AttrIdx < Count; AttrIdx++)
        {
            ...
            if (CopyResult == EOS_EResult::EOS_Success)
            {
                ...
                UE_LOG(LogRedpointEOS, Verbose, TEXT("LobbyInterface:   - Key: %s"), UTF8_TO_TCHAR(Attr->Data->Key));
                UE_LOG(LogRedpointEOS, Verbose, TEXT("LobbyInterface:     Type: %s"), *TypeStr);
                UE_LOG(LogRedpointEOS, Verbose, TEXT("LobbyInterface:     Value: %s"), *ValueStr);
    ...
    UE_LOG(LogRedpointEOS, Verbose, TEXT("LobbyInterface: Members:"));
    ...
    for (uint32_t MemberIdx = 0; MemberIdx < MemberCount; MemberIdx++)
    {
        ...
        if (!EOSString_ProductUserId::IsValid(MemberId))
        {
            UE_LOG(LogRedpointEOS, Warning, TEXT("LobbyInterface:   - User ID: (invalid!)"));
            continue;
        }
        ...
        UE_LOG(LogRedpointEOS, Verbose, TEXT("LobbyInterface:   - User ID: %s"), *MemberIdEOS->ToString());
        UE_LOG(LogRedpointEOS, Verbose, TEXT("LobbyInterface:     Type: %s"), *Designation);
        UE_LOG(LogRedpointEOS, Verbose, TEXT("LobbyInterface:     Attributes:"));
            ...
            for (uint32_t AttrIdx = 0; AttrIdx < Count; AttrIdx++)
            {
                ...
                if (CopyResult == EOS_EResult::EOS_Success)
                {
                    ...
                    UE_LOG(
                        LogRedpointEOS,
                        Verbose,
                        TEXT("LobbyInterface:     - Key: %s"),
                        UTF8_TO_TCHAR(Attr->Data->Key));
                    UE_LOG(LogRedpointEOS, Verbose, TEXT("LobbyInterface:       Type: %s"), *TypeStr);
                    UE_LOG(LogRedpointEOS, Verbose, TEXT("LobbyInterface:       Value: %s"), *ValueStr);
    ...
    UE_LOG(LogRedpointEOS, Verbose, TEXT("LobbyInterface: --- Lobby State: %s ( End ) ---"), ANSI_TO_TCHAR(InLobbyId));
}
...
void FOnlineLobbyInterfaceEOS::Handle_LobbyUpdateReceived(const EOS_Lobby_LobbyUpdateReceivedCallbackInfo *Data)
{
    UE_LOG(
        LogRedpointEOS,
        Verbose,
        TEXT("LobbyInterface: LobbyUpdateReceived: (Lobby) %s"),
        ANSI_TO_TCHAR(Data->LobbyId));
    ...
    FOnLobbyCreateOrConnectComplete OnComplete)
{
    ...
    bool bBucketIdDefaulted = false;
    if (EOSTransaction->SetMetadata.Contains(EOS_LOBBY_SEARCH_BUCKET_ID) &&
        EOSTransaction->SetMetadata[EOS_LOBBY_SEARCH_BUCKET_ID].GetType() == EOnlineKeyValuePairDataType::String)
    {
        FString BucketId;
        EOSTransaction->SetMetadata[EOS_LOBBY_SEARCH_BUCKET_ID].GetValue(BucketId);
        EOSString_SessionModification_BucketId::AllocateToCharBuffer(BucketId, Opts.BucketId);
    }
    else
    {
        Opts.BucketId = "Default";
        bBucketIdDefaulted = true;
    }
    ...
    EOSRunOperation<EOS_HLobby, EOS_Lobby_CreateLobbyOptions, EOS_Lobby_CreateLobbyCallbackInfo>(
        ...
        [WeakThis = GetWeakThis(this),
         ...
         bEnableEcho,
         bBucketIdDefaulted,
         Opts](const EOS_Lobby_CreateLobbyCallbackInfo *Info) {
            if (!bBucketIdDefaulted)
            {
                EOSString_SessionModification_BucketId::FreeFromCharBufferConst(Opts.BucketId);
            }
                    ...
                    [WeakThis = GetWeakThis(This),
                     ...
                     bEnableEcho](const EOS_Lobby_UpdateLobbyCallbackInfo *Info) {
                        ...
                        if (auto This = PinWeakThis(WeakThis))
                        {
                            ...
                            if (bEnableEcho)
                            {
                                This->VoiceManager->MarkPartyOrLobbyAsEchoEnabled(
                                    EOSUser,
                                    MakeShared<FOnlinePartyIdEOS>(Info->LobbyId));
                            }
                            ...
                            This->VoiceManager->UserManuallyJoinedLobby(
                                *EOSUser,
                                *StaticCastSharedPtr<const FOnlinePartyIdEOS>(ResultLobby->Id));
    ...
    FOnLobbyOperationComplete OnComplete)
{
    ...
    for (const auto &Metadata : Transaction.SetMetadata)
    {
        if (Metadata.Key == EOS_LOBBY_SEARCH_BUCKET_ID)
        {
            auto BucketIdValue =
                EOSString_LobbyModification_AttributeStringValue::ToUtf8String(Metadata.Value.ToString());
            if (BucketIdValue.Result != EOS_EResult::EOS_Success)
            {
                OnComplete.ExecuteIfBound(
                    ConvertError(
                        TEXT("EOSString_LobbyModification_AttributeStringValue::ToUtf8String"),
                        FString::Printf(TEXT("The bucket ID in attribute '%s' has an invalid value"), *Metadata.Key),
                        BucketIdValue.Result),
                    *EOSUser);
                return true;
            }

            EOS_LobbyModification_SetBucketIdOptions SetBucketOpts = {};
            SetBucketOpts.ApiVersion = EOS_LOBBYMODIFICATION_SETBUCKETID_API_LATEST;
            SetBucketOpts.BucketId = BucketIdValue.GetAsChar();

            EOS_EResult SetBucketResult = EOS_LobbyModification_SetBucketId(ModHandle, &SetBucketOpts);
            if (SetBucketResult != EOS_EResult::EOS_Success)
            {
                OnComplete.ExecuteIfBound(
                    ConvertError(
                        TEXT("EOS_LobbyModification_SetBucketId"),
                        FString::Printf(
                            TEXT("Failed to set bucket ID '%s' from attribute '%s' to modification transaction"),
            ...
            // We've handled this special attribute, proceed to next attribute in the list.
            continue;
        }
    ...
    FOnLobbyOperationComplete OnComplete)
{
    ...
    EOSRunOperation<EOS_HLobby, EOS_Lobby_DestroyLobbyOptions, EOS_Lobby_DestroyLobbyCallbackInfo>(
        ...
        &Opts,
        &EOS_Lobby_DestroyLobby,
        [WeakThis = GetWeakThis(this), OnComplete, EOSUser, EOSLobbyId](
    ...
    FOnLobbyCreateOrConnectComplete OnComplete)
{
    ...
    auto ConnectWithHandle = [EOSUser](
                                 ...
                                 const FEOSLobbyDetailsHandle &LobbyDetails,
        ...
        EOSRunOperation<EOS_HLobby, EOS_Lobby_JoinLobbyOptions, EOS_Lobby_JoinLobbyCallbackInfo>(
            ...
            &Opts,
            &EOS_Lobby_JoinLobby,
            [WeakThis = GetWeakThis(This), LobbyDetails, OnComplete, EOSUser](
    ...
    // for lobbies that the EOS SDK has in memory or are in the search results.
    {
        ...
        auto Result = CopyLobbyDetailsHandle(*EOSUser, *EOSLobbyId, LobbyDetails);
        if (Result == EOS_EResult::EOS_Success)
        {
            ConnectWithHandle(this->AsShared(), EOSUser, LobbyDetails, OnComplete);
            return true;
        }
    }
            ...
            [WeakThis = GetWeakThis(this), EOSUser, OnComplete, LobbySearch, ConnectWithHandle](
                ...
                if (auto This = PinWeakThis(WeakThis))
                {
                    ...
                    EOS_EResult CopyResult =
                        LobbyDetails.WithWrite<EOS_EResult>(This->EOSSubsystem, [&](EOS_HLobbyDetails *WriteTarget) {
                            return EOS_LobbySearch_CopySearchResultByIndex(LobbySearch, &CopyOpts, WriteTarget);
                        });
                    if (CopyResult != EOS_EResult::EOS_Success)
                    {
                        EOS_LobbySearch_Release(LobbySearch);
                    ...
                    ConnectWithHandle(This.ToSharedRef(), EOSUser, LobbyDetails, OnComplete);
                }
            });
        return true;
    }
    ...
    FOnLobbyOperationComplete OnComplete)
{
    ...
    EOSRunOperation<EOS_HLobby, EOS_Lobby_LeaveLobbyOptions, EOS_Lobby_LeaveLobbyCallbackInfo>(
        ...
        [WeakThis = GetWeakThis(this), OnComplete, EOSUser, EOSLobbyId](const EOS_Lobby_LeaveLobbyCallbackInfo *Data) {
            if (auto This = PinWeakThis(WeakThis))
            {
                if (Data->ResultCode == EOS_EResult::EOS_Success || Data->ResultCode == EOS_EResult::EOS_NotFound)
                {
                    ...
                    This->VoiceManager->UserManuallyLeftLobby(*EOSUser, *EOSLobbyId);
                    This->Internal_RemoveMemberFromLobby(*EOSUser, *EOSLobbyId);
                    OnComplete.ExecuteIfBound(OnlineRedpointEOS::Errors::Success(), *EOSUser);
                }
                else
    ...
    FOnLobbySearchComplete OnComplete)
{
    ...
    {
        if (Filter.Key == FString("EOS_BucketId"))
        {
            auto BucketIdValue =
                EOSString_LobbyModification_AttributeStringValue::ToUtf8String(Filter.Value.ToString());
            if (BucketIdValue.Result != EOS_EResult::EOS_Success)
            {
                OnComplete.ExecuteIfBound(
                    ConvertError(
    ...
    EOSRunOperation<EOS_HLobbySearch, EOS_LobbySearch_FindOptions, EOS_LobbySearch_FindCallbackInfo>(
        ...
        [WeakThis = GetWeakThis(this), LobbySearch, EOSUser, OnComplete](const EOS_LobbySearch_FindCallbackInfo *Info) {
            if (auto This = PinWeakThis(WeakThis))
            {
                ...
                {
                    This->LastSearchResultsByUser.Add(*EOSUser, TMap<FString, FEOSLobbyDetailsHandle>());
                }
                ...
                for (uint32_t i = 0; i < Count; i++)
                {
                        ...
                        EOS_EResult::EOS_Success)
                    {
                        ...
                        {
                            FEOSLobbyDetailsHandle LobbyDetails = FEOSLobbyDetailsHandle(This->EOSSubsystem, Details);

                            auto LobbyId = MakeShared<FOnlinePartyIdEOS>(LobbyInfo->LobbyId);
                            Results.Add(LobbyId);
                            LastSearchResults.Add(LobbyId->ToString(), LobbyDetails);
                            bKeep = true;

#if defined(EOS_ENABLE_STATE_DIAGNOSTICS) && EOS_ENABLE_STATE_DIAGNOSTICS
                            This->DumpLobbyAttributeState(
    ...
    FEOSLobbyDetailsHandle &LobbyDetails)
{
    ...
    EOS_EResult LiveCopyResult =
        LobbyDetails.WithWrite<EOS_EResult>(this->EOSSubsystem, [&](EOS_HLobbyDetails *WriteTarget) {
            return EOS_Lobby_CopyLobbyDetailsHandle(this->EOSLobby, &CopyOpts, WriteTarget);
        });
    ...
    FVariantData &OutMetadataValue)
{
    ...
    if (Result == EOS_EResult::EOS_Success)
    {
        if (MetadataKey == EOS_LOBBY_SEARCH_BUCKET_ID)
        {
            EOS_LobbyDetails_CopyInfoOptions CopyOpts = {};
            CopyOpts.ApiVersion = EOS_LOBBYDETAILS_COPYINFO_API_LATEST;

            if (EOS_LobbyDetails_CopyInfo(*LobbyDetails, &CopyOpts, &Info) == EOS_EResult::EOS_Success)
            {
                OutMetadataValue = FVariantData(EOSString_SessionModification_BucketId::FromAnsiString(Info->BucketId));
                DidSet = true;
            }
        }
        else
