...
TSharedPtr<const FCrossPlatformAccountId> FEpicGamesCrossPlatformAccountId::ParseFromString(const FString &In)
{
    ...
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("Malformed Epic account ID component of unique net ID: %s"), *In);
        return nullptr;
    }
...
FString CheckForDesktopCrossplayError(EOS_HPlatform InPlatform)
{
    ...
    // This SDK version no longer requires the cross-play service.
    return TEXT("");
#elif EOS_VERSION_AT_LEAST(1, 15, 0)
#if PLATFORM_WINDOWS
