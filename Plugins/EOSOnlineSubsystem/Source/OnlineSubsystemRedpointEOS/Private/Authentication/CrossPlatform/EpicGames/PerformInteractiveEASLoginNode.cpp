...
FPerformInteractiveEASLoginNode::FPerformInteractiveEASLoginNode(bool bInSkipLogoutNodeOnCleanup)
    : bSkipLogoutNodeOnCleanup(bInSkipLogoutNodeOnCleanup)
    , Widget()
{
}
    ...
    const FAuthenticationGraphNodeOnDone &OnDone)
{
    ...
    if (!this->bSkipLogoutNodeOnCleanup)
    {
    ...
    FAuthenticationGraphNodeOnDone OnDone)
{
    ...
    if (!DesktopCrossplayError.IsEmpty())
    {
        ...
        else
        {
            ...
            UE_LOG(LogRedpointEOS, Warning, TEXT("%s"), *DesktopCrossplayError);
            OnDone.ExecuteIfBound(EAuthenticationGraphNodeResult::Continue);
        }
        return;
    }
    ...
    {
#if PLATFORM_WINDOWS || PLATFORM_MAC || PLATFORM_IOS || PLATFORM_ANDROID || PLATFORM_LINUX || EOS_VERSION_AT_LEAST(1, 16, 0)
