    ...
    TSharedPtr<IOnlineIdentity, ESPMode::ThreadSafe> OSSIdentity)
{
    if (!Error.bSucceeded)
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("Unable to sign user in through online subsystem prompt."));
    ...
    FAuthenticationGraphNodeOnDone InOnDone)
{
    ...
    {
        UE_LOG(LogRedpointEOS, Verbose, TEXT("Local user ID is not valid, requesting sign in via external UI if possible..."));
        ...
        if (OSSExternalUI.IsValid())
        {
            if (!OSSExternalUI->ShowLoginUI(
                    ...
                    FOnLoginUIClosedDelegate::CreateSP(
                        ...
                        OSSIdentity)))
            {
                UE_LOG(LogRedpointEOS, Error, TEXT("User is not signed in, and could not show external UI sign-in prompt."));
                InState->ErrorMessages.Add(
        ...
        UE_LOG(LogRedpointEOS, Verbose, TEXT("External UI interface is invalid, calling DoLogin directly..."));
    }
