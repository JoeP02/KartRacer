    ...
    const FAuthenticationGraphNodeOnDone &OnDone)
{
    ...
    UE_LOG(LogRedpointEOS, Verbose, TEXT("Authentication graph is starting"));
    UE_LOG(LogRedpointEOS, Verbose, TEXT("Authentication graph entering node: %s"), *Start->GetDebugName());
...
{
    UE_LOG(LogRedpointEOS, Verbose, TEXT("Authentication graph exiting node: %s"), *this->Start->GetDebugName());
    ...
    if (Result == EAuthenticationGraphNodeResult::Error)
    {
        if (ResultingState->CleanupNodesOnError.Num() > 0)
        {
            ...
            UE_LOG(
                ...
                *Node->GetDebugName());
            UE_LOG(LogRedpointEOS, Verbose, TEXT("Authentication graph entering node: %s"), *Node->GetDebugName());
            Node->Execute(
                ResultingState,
    ...
    UE_LOG(LogRedpointEOS, Verbose, TEXT("Authentication graph is complete"));
    ...
    EAuthenticationGraphNodeResult OriginalResult)
{
    ...
    if (Result == EAuthenticationGraphNodeResult::Error)
    {
        UE_LOG(LogRedpointEOS, Warning, TEXT("Authentication graph cleanup encountered an error, continuing cleanup anyway"));
    }
