...
FP2PMessagingHub::FP2PMessagingHub(
...
void FP2PMessagingHub::OnLoginStatusChanged(
    int32 LocalUserNum,
    ...
    const FUniqueNetId &NewId)
{
    if (OldStatus == ELoginStatus::NotLoggedIn && NewStatus == ELoginStatus::LoggedIn)
    {
        ...
        if (!this->LocalUserSockets.Contains(LocalUserNum))
        {
            ...
            Socket->OnConnectionClosed.BindSP(this, &FP2PMessagingHub::OnConnectionClosed);
            Socket->OnConnectionAccepted.BindSP(this, &FP2PMessagingHub::OnConnectionAccepted, LocalUserNum);
        ...
        {
            UE_LOG(LogRedpointEOS, Warning, TEXT("Local user already had a message hub socket listening!"));
        }
    }
    else if (OldStatus == ELoginStatus::LoggedIn && NewStatus == ELoginStatus::NotLoggedIn)
    {
        ...
        {
            UE_LOG(LogRedpointEOS, Warning, TEXT("Local user does not have a message hub socket listening!"));
        }
    }
}

void FP2PMessagingHub::OnConnectionAccepted(
...
bool FP2PMessagingHub::Tick(float DeltaSeconds)
{
                ...
                ->RecvFrom(this->RecvBuffer, MSG_HUB_MAX_SIZE, BytesRead, *ReceivedAddr, ESocketReceiveFlags::None))
        {
            ...
            else
            {
                ...
                if (this->MessagesPendingAck.Contains(MsgId))
                {
                    ...
                    {
                        UE_LOG(LogRedpointEOS, Warning, TEXT("Received ack for incorrect owner in EOS messaging hub."));
                    }
                }
                else
                {
                    UE_LOG(LogRedpointEOS, Warning, TEXT("Missing message ID to ack in EOS messaging hub."));
                }
            }
        }
    }
...
void FP2PMessagingHub::OnConnectionClosed(
    const TSharedRef<class ISocketEOS> &ListeningSocket,
...
void FP2PMessagingHub::RegisterEvents()
{
    for (int32 i = 0; i < MAX_LOCAL_PLAYERS; i++)
    {
        this->Identity->AddOnLoginStatusChangedDelegate_Handle(
            i,
            FOnLoginStatusChangedDelegate::CreateSP(this, &FP2PMessagingHub::OnLoginStatusChanged));
    }

    this->TickHandle = FTSTicker::GetCoreTicker().AddTicker(FTickerDelegate::CreateSP(this, &FP2PMessagingHub::Tick));
}

FOnMessagingHubReceived &FP2PMessagingHub::OnMessageReceived()
{
    return this->OnMessageReceivedDelegate;
}

void FP2PMessagingHub::SendMessage(
    ...
    const FOnMessagingHubSent &Delegate)
{
    ...
    {
        UE_LOG(LogRedpointEOS, Warning, TEXT("Sender for EOS messaging hub is not a locally signed in user."));
