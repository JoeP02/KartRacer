    ...
    TArray<uint8> &FileContents)
{
    ...
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("GetFileContents: User ID was not an EOS user ID"));
        return false;
    }
...
bool FOnlineUserCloudInterfaceEOS::ClearFiles(const FUniqueNetId &UserId)
{
    if (UserId.GetType() != REDPOINT_EOS_SUBSYSTEM)
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("ClearFiles: User ID was not an EOS user ID"));
        return false;
    }
...
bool FOnlineUserCloudInterfaceEOS::ClearFile(const FUniqueNetId &UserId, const FString &FileName)
{
    if (UserId.GetType() != REDPOINT_EOS_SUBSYSTEM)
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("ClearFile: User ID was not an EOS user ID"));
        return false;
    }
...
void FOnlineUserCloudInterfaceEOS::EnumerateUserFiles(const FUniqueNetId &UserId)
{
    if (UserId.GetType() != REDPOINT_EOS_SUBSYSTEM)
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("EnumerateUserFiles: User ID was not an EOS user ID"));
...
void FOnlineUserCloudInterfaceEOS::GetUserFileList(const FUniqueNetId &UserId, TArray<FCloudFileHeader> &UserFiles)
{
    if (UserId.GetType() != REDPOINT_EOS_SUBSYSTEM)
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("GetUserFileList: User ID was not an EOS user ID"));
...
bool FOnlineUserCloudInterfaceEOS::ReadUserFile(const FUniqueNetId &UserId, const FString &FileName)
{
    if (UserId.GetType() != REDPOINT_EOS_SUBSYSTEM)
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("ReadUserFile: User ID was not an EOS user ID"));
        return false;
    }
    ...
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("ReadUserFile: File download already in progress for: %s"), *FileName);
        return false;
    }
    ...
    bool bCompressBeforeUpload)
{
    if (UserId.GetType() != REDPOINT_EOS_SUBSYSTEM)
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("WriteUserFile: User ID was not an EOS user ID"));
        return false;
    }
    ...
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("WriteUserFile: File upload already in progress for: %s"), *FileName);
        return false;
    }
...
void FOnlineUserCloudInterfaceEOS::CancelWriteUserFile(const FUniqueNetId &UserId, const FString &FileName)
{
    if (UserId.GetType() != REDPOINT_EOS_SUBSYSTEM)
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("CancelWriteUserFile: User ID was not an EOS user ID"));
        return;
    }
    ...
    if (!this->WriteOperations.Contains(OpKey))
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("CancelWriteUserFile: No write operation in progress for: %s"), *FileName);
        return;
    }
    ...
    bool bShouldLocallyDelete)
{
    if (UserId.GetType() != REDPOINT_EOS_SUBSYSTEM)
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("DeleteUserFile: User ID was not an EOS user ID"));
        return false;
    }
...
bool FOnlineUserCloudInterfaceEOS::RequestUsageInfo(const FUniqueNetId &UserId)
{
    UE_LOG(
        LogRedpointEOS,
        Error,
        TEXT("RequestUsageInfo: EOS does not support querying usage info for Player Data Storage"));
    return false;
}
