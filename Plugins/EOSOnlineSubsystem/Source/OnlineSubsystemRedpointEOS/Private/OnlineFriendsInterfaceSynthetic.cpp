    ...
    const std::function<void(bool OutValue)> &OnDone)
{
    if (auto Friends = InFriends.Pin())
    {
        ...
        {
            if (DelegatedIdentity->GetLoginStatus(LocalUserNum) != ELoginStatus::NotLoggedIn)
            {
                        ...
                        if (!bWasSuccessful)
                        {
                            FOnlineError Err = OnlineRedpointEOS::Errors::UnexpectedError(
                                ...
                                FString::Printf(
                                    ...
                                    *ErrorStr));
                            UE_LOG(LogRedpointEOSFriends, Error, TEXT("%s"), *Err.ToLogString());
                        }
        ...
        for (const auto &FriendRef : List)
        {
            ...
            if (FriendInfo->HasExternalInfo)
            {
                UE_LOG(
                    ...
                    VeryVerbose,
                    TEXT("%s -> %s: Using %s as the external account ID with type %d."),
                    *FriendInfo->SubsystemName.ToString(),
                    *FriendRef->GetUserId()->ToString(),
                    *FriendInfo->ExternalInfo.AccountId,
                    FriendInfo->ExternalInfo.AccountIdType);
            }
            else
            {
                UE_LOG(
                    LogRedpointEOSFriends,
                    VeryVerbose,
                    TEXT("%s -> %s: No external account information could be found for this friend."),
                    *FriendInfo->SubsystemName.ToString(),
                    *FriendRef->GetUserId()->ToString());
            }

            FriendInfo->FetchedAvatarUrl = TEXT("");
            FriendInfo->LastFetchedAvatarUrl = FDateTime::MinValue();

            if (this->RuntimePlatform->UseCrossPlatformFriendStorage())
            {
                // If we are using cross-platform friends storage, try to pull the avatar information out
                // of the cross-platform friends database now. This will prevent us from trying to re-fetch
                // avatars when they're already up-to-date.
                auto &DbCache = this->FriendState[LocalUserNum]->FriendDatabase->GetDelegatedSubsystemCachedFriends();
                if (DbCache.Contains(FriendInfo->SubsystemName))
                {
                    auto &DbSubsystemCache = DbCache[FriendInfo->SubsystemName];
                    if (DbSubsystemCache.Contains(FriendRef->GetUserId()->ToString()))
                    {
                        auto &DbEntry = DbSubsystemCache[FriendRef->GetUserId()->ToString()];
                        FriendInfo->FetchedAvatarUrl = DbEntry.AccountAvatarUrl;
                        FriendInfo->LastFetchedAvatarUrl = DbEntry.AccountAvatarUrlLastFetched;
                        UE_LOG(
                            LogRedpointEOSFriends,
                            VeryVerbose,
                            TEXT("%s -> %s: Retrieved existing avatar URL from cross-platform friend storage."),
                            *FriendInfo->SubsystemName.ToString(),
                            *FriendRef->GetUserId()->ToString());
                    }
                    else
                    {
                        UE_LOG(
                            LogRedpointEOSFriends,
                            VeryVerbose,
                            TEXT("%s -> %s: Cross-platform friend storage does not contain an entry for this friend "
                                 "yet."),
                            *FriendInfo->SubsystemName.ToString(),
                            *FriendRef->GetUserId()->ToString());
                    }
                ...
                else
                {
                    UE_LOG(
                        ...
                        VeryVerbose,
                        TEXT(
                            "%s -> %s: Cross-platform friend storage does not contain any entries for this subsystem."),
                        *FriendInfo->SubsystemName.ToString(),
                        *FriendRef->GetUserId()->ToString());
                }
            }
        ...
        FMultiOperation<TSharedPtr<FFriendInfo>, bool>::Run(
            ...
            [WeakThis = GetWeakThis(this),
             LocalUserNum](const TSharedPtr<FFriendInfo> &Entry, const std::function<void(bool)> &OnDone) {
                if (auto This = PinWeakThis(WeakThis))
                {
                    // Only do the fetch if we don't have an avatar for this person, or
                    // the avatar we do have is older than 24 hours.
                    if (Entry->FetchedAvatarUrl.IsEmpty() ||
                        Entry->LastFetchedAvatarUrl < FDateTime::UtcNow() - FTimespan::FromDays(1))
                    {
                        if (Entry->FetchedAvatarUrl.IsEmpty())
                        {
                            UE_LOG(
                                LogRedpointEOSFriends,
                                VeryVerbose,
                                TEXT("%s -> %s: Fetching avatar for this friend because there is no currently cached "
                                     "avatar URL."),
                                *Entry->SubsystemName.ToString(),
                                *Entry->ExternalInfo.AccountId);
                        }
                        else
                        {
                            UE_LOG(
                                LogRedpointEOSFriends,
                                VeryVerbose,
                                TEXT("%s -> %s: Fetching avatar for this friend because the avatar was fetched %f "
                                     "seconds ago and the threshold for refreshing avatars is %f seconds."),
                                *Entry->SubsystemName.ToString(),
                                *Entry->ExternalInfo.AccountId,
                                (FDateTime::UtcNow() - Entry->LastFetchedAvatarUrl).GetTotalSeconds(),
                                FTimespan::FromDays(1).GetTotalSeconds());
                        }
                        return This->Op_ResolveAvatarForFriends(LocalUserNum, Entry, OnDone);
                    }
                    else
                    {
                        // Our cached version is already good enough.
                        UE_LOG(
                            LogRedpointEOSFriends,
                            VeryVerbose,
                            TEXT("%s -> %s: The current avatar URL is already up-to-date for this friend."),
                            *Entry->SubsystemName.ToString(),
                            *Entry->ExternalInfo.AccountId);
                        return false;
                    }
        ...
        FOnGetAvatarUrlComplete::CreateLambda(
            [Friend, OnDone](const bool &bWasSuccessful, const FString &ResultAvatarUrl) {
                if (bWasSuccessful)
                {
                    ...
                    Friend->LastFetchedAvatarUrl = FDateTime::UtcNow();
                }
                OnDone(bWasSuccessful);
            }));
}
    ...
    if (this->RuntimePlatform->UseCrossPlatformFriendStorage())
    {
        ...
        for (auto i = 0; i < OutValues.Num(); i++)
        {
            ...
            for (const auto &FriendRef : OutValues[i])
            {
                ...
                if (UserIdToFriendInfo.Contains(*FriendRef->GetUserId()))
                {
                    ...
                    CachedFriend.AccountAvatarUrlLastFetched =
                        UserIdToFriendInfo[*FriendRef->GetUserId()]->LastFetchedAvatarUrl;
                }
        ...
        for (const auto &KV : DbCache)
        {
            ...
            for (const auto &FV : KV.Value)
            {
                ...
                FriendInfo->FetchedAvatarUrl = FV.Value.AccountAvatarUrl;
                FriendInfo->LastFetchedAvatarUrl = FV.Value.AccountAvatarUrlLastFetched;
                if (FriendInfo->HasExternalInfo)
                {
            ...
            for (auto i = 0; i < MAX_LOCAL_PLAYERS; i++)
            {
                this->OnFriendsChangeDelegates.Add(TTuple<
                    ...
                    Friends->AddOnFriendsChangeDelegate_Handle(
                        i,
                        FOnFriendsChangeDelegate::CreateLambda([WeakThis = GetWeakThis(this),
                                                                i,
                                                                SubsystemName = FriendsWeak.SubsystemName]() {
                            if (auto This = PinWeakThis(WeakThis))
                            {
                                if (This->PendingReadFriendsListFromPropagation.Contains(i))
                                {
                                    bool bShouldIgnoreOnFriendsChangeEvent = false;
                                    for (const auto &Integration : This->RuntimePlatform->GetIntegrations())
                                    {
                                        Integration->ShouldIgnoreOnFriendsChangeEventDuringReadFriendsOperation(
                                            SubsystemName,
                                            bShouldIgnoreOnFriendsChangeEvent);
                                    }

                                    if (bShouldIgnoreOnFriendsChangeEvent)
                                    {
                                        UE_LOG(
                                            LogRedpointEOSFriends,
                                            Verbose,
                                            TEXT("FOnlineFriendsInterfaceSynthetic: Ignoring OnFriendsChange event "
                                                 "from delegated subsystem for user %d, because a runtime integration "
                                                 "indicated it is an incorrect event fire as a result of calling "
                                                 "OnReadFriends."),
                                            i);
                                    }
                                    else
                                    {
                                        UE_LOG(
                                            LogRedpointEOSFriends,
                                            Verbose,
                                            TEXT("FOnlineFriendsInterfaceSynthetic: Received OnFriendsChange event "
                                                 "from delegated subsystem for user %d, but there is already a "
                                                 "ReadFriendsList operation in progress. Scheduling a second call "
                                ...
                                else
                                {
                                    UE_LOG(
                                        LogRedpointEOSFriends,
                                        Verbose,
                                        TEXT("FOnlineFriendsInterfaceSynthetic: Received OnFriendsChange event from "
                                ...
                                if (auto This = PinWeakThis(WeakThis))
                                {
                                    ...
                                    This->MessagingHub->SendMessage(
                                        ...
                                        *SenderId,
                                        ...
                                        TEXT(""),
                                        FOnMessagingHubSent::CreateLambda([WeakThis = GetWeakThis(This),
                                                                           SenderId,
                                                                           ReceiverId,
                ...
                case ESerializedPendingFriendRequestMode::InboundPendingSendAcceptance: {
                    this->MessagingHub->SendMessage(
                        ...
                        *TargetId,
                        ...
                        TEXT(""),
                        FOnMessagingHubSent::CreateLambda(
...
FOnlineFriendsInterfaceSynthetic::FOnlineFriendsInterfaceSynthetic(
    ...
    const TSharedRef<FOnlineUserCloudInterfaceEOS, ESPMode::ThreadSafe> &InUserCloudImpl,
    const TSharedRef<class IMessagingHub> &InMessagingHub,
