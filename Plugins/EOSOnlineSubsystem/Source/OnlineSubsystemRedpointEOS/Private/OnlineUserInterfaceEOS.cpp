...
bool FOnlineUserInterfaceEOS::QueryUserInfo(int32 LocalUserNum, const TArray<TSharedRef<const FUniqueNetId>> &UserIds)
{
    ...
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("QueryUserInfo: Local user is not logged in"));
        return false;
    }
    ...
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("QueryUserInfo: Local user is not using the Epic subsystem"));
        return false;
    }
...
bool FOnlineUserInterfaceEOS::GetAllUserInfo(int32 LocalUserNum, TArray<TSharedRef<FOnlineUser>> &OutUsers)
{
    if (!this->CachedUsers.Contains(LocalUserNum))
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("GetAllUserInfo: No users have been cached for this user"));
        return false;
    }
    ...
    const FOnQueryUserMappingComplete &Delegate)
{
    ...
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("QueryUserIdMapping: This function can not be used on a dedicated server"));
        return false;
    }

    if (UserId.GetType() != REDPOINT_EOS_SUBSYSTEM)
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("Expected local user ID to be an EOS user."));
        return false;
    }
    ...
    UE_LOG(LogRedpointEOS, Error, TEXT("QueryUserIdMapping: Local user is not signed into an Epic Games account"));
    return false;
#else
    UE_LOG(LogRedpointEOS, Error, TEXT("QueryUserIdMapping: This function can not be used on a dedicated server"));
    return false;
    ...
    TArray<TSharedPtr<const FUniqueNetId>> &OutIds)
{
    if (QueryOptions.bLookupByDisplayName)
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("bLookupByDisplayName must be false"));
        return;
    }
    ...
    if (!this->CachedMappings.Contains(MappingKey))
    {
        UE_LOG(LogRedpointEOS, Error, TEXT("QueryExternalIdMappings has not been called"));
        return;
    }
