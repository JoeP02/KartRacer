...
void FEOSDedicatedServerAntiCheat::EnsureSessionIsCurrent(FAntiCheatSession &Session, const TCHAR *FunctionName) const
{
...
{
    UE_LOG(LogRedpointEOSAntiCheat, Verbose, TEXT("Dedicated server Anti-Cheat: %p: Shutdown"), this);
...
#define FUNCTION_CONTEXT_MACRO                                                                                         \
    ...
    "%u)"
#define FUNCTION_CONTEXT_ARGS this, *Session.ToString(), *UserId.ToString(), ClientType, ClientPlatform
    ...
    if (bShouldRegister)
    {
        ...
        Opts.UserId = UserId.GetProductUserId();
#else
...
#if !EOS_VERSION_AT_LEAST(1, 15, 4)
        EOSString_ProductUserId::FreeFromCharBuffer(Opts.AccountId);
#endif
        if (Result != EOS_EResult::EOS_Success)
        {
...
#define FUNCTION_CONTEXT_ARGS this, *Session.ToString(), *UserId.ToString()
...
#define FUNCTION_CONTEXT_MACRO                                                                                         \
    ...
    "..., Size: %u)"
#define FUNCTION_CONTEXT_ARGS this, *Session.ToString(), *SourceUserId.ToString(), *TargetUserId.ToString(), Size
