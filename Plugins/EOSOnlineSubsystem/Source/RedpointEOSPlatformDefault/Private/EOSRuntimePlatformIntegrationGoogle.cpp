    ...
    EOS_Connect_ExternalAccountInfo *InExternalInfo)
{
    ...
    IOnlineSubsystem *OSS = Online::GetSubsystem(InWorld.Get(), FName(TEXT("RedpointGoogle")));
    if (OSS != nullptr)
    {
...
{
    return InUserId.GetType().IsEqual(FName(TEXT("RedpointGoogle")));
}

Redpoint::EOS::Core::FExternalAccountIdInfo FEOSRuntimePlatformIntegrationGoogle::GetExternalAccountId(
