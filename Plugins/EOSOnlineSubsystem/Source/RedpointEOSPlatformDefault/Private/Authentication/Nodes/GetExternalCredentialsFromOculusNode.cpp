    ...
    FOnlineExternalCredentialsRefreshComplete OnComplete)
{
    ...
    UE_LOG(LogRedpointEOSPlatformDefault, Verbose, TEXT("Obtained refresh Oculus user proof token: %s"), *this->ProofToken);
    ...
    FAuthenticationGraphNodeOnDone InOnDone)
{
    if (bIsError || ovr_Message_IsError(InHandle))
    {
        UE_LOG(LogRedpointEOSPlatformDefault, Error, TEXT("Authentication with Oculus failed."));
    ...
    UE_LOG(LogRedpointEOSPlatformDefault, Verbose, TEXT("Authenticating with Oculus user proof token: %s"), *OculusToken);
