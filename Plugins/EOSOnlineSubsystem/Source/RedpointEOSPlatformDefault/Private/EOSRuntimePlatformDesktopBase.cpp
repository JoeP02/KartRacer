...
{
    UE_LOG(LogRedpointEOSPlatformDefault, Verbose, TEXT("EOS SDK Dynamic Load - Starting dynamic load of EOS SDK..."));
...
#error Platform not supported by EOSDynamicLoaderDefault.
    ...
    for (const auto &BaseSearchPath : SearchPaths)
    {
        ...
        UE_LOG(LogRedpointEOSPlatformDefault, Verbose, TEXT("EOS SDK Dynamic Load - DLL found at: %s"), *SearchPath);
...
const TArray<TSharedPtr<Redpoint::EOS::Core::IRuntimePlatformIntegration>> &FEOSRuntimePlatformDesktopBase::
    GetIntegrations() const
{
