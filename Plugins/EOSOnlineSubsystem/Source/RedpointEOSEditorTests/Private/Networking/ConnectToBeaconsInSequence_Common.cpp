    ...
    for (const FWorldContext &WorldContext : GEngine->GetWorldContexts())
    {
            ...
            WorldContext.PIEInstance == this->ClientPIEInstance)
        {
            for (TActorIterator<AEOSTestbedBeaconClient> It(WorldContext.World()); It; ++It)
            {
                ...
                // Beacons don't destroy themselves properly after DestroyBeacon is called in 5.2, but
                // they still do disconnect from the server. Just check the connection status instead.
                if (It->GetConnectionState() == EBeaconConnectionState::Closed ||
                    It->GetConnectionState() == EBeaconConnectionState::Invalid)
                {
                    // Beacon is disconnected. Try to call Destroy and return true.
                    It->Destroy();
                    return true;
                }

                // Beacon is still connected.
                return false;
#else
                // Beacon still exists.
                return false;
