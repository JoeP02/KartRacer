...
public class RedpointEOSPlatform_Win32 : IRedpointEOSPlatform
{
    ...
    public void ConfigureModule(string SdkBase, string ModuleBase, ModuleRules Module, ReadOnlyTargetRules Target)
    {
        ...
        Module.RuntimeDependencies.Add("$(TargetOutputDir)/RedpointEOS/EOSSDK-Win32-Shipping.dll", Path.Combine(SdkBase, "Bin", "EOSSDK-Win32-Shipping.dll"));
        if (File.Exists(Path.Combine(SdkBase, "Bin", "x86", "xaudio2_9redist.dll")))
        {
            Module.RuntimeDependencies.Add("$(TargetOutputDir)/RedpointEOS/x86/xaudio2_9redist.dll", Path.Combine(SdkBase, "Bin", "x86", "xaudio2_9redist.dll"));
        }
...
public class RedpointEOSPlatform_Win64 : IRedpointEOSPlatform
{
	...
	public void ConfigureModule(string SdkBase, string ModuleBase, ModuleRules Module, ReadOnlyTargetRules Target)
	{
		...
		if (File.Exists(Path.Combine(SdkBase, "Bin", "x64", "xaudio2_9redist.dll")))
		{
...
public class RedpointEOSPlatform_IOS : IRedpointEOSPlatform
{
	...
	public void ConfigureModule(string SdkBase, string ModuleBase, ModuleRules Module, ReadOnlyTargetRules Target)
	{
		...
		string FrameworkDirectory = Path.Combine(SdkBase, "Bin", "IOS", "EOSSDK.framework");
		string FrameworkPath = Path.Combine(SdkBase, "Bin", "IOS", "EOSSDK.embeddedframework.zip");
...
#else
            Console.WriteLine("EOSSDK (IOS): Executing /usr/bin/zip -r '" + FrameworkPath + "' . (in " + target + ")");
            var zipStart = new ProcessStartInfo
            {
                FileName = "/usr/bin/zip",
                Arguments = "-r '" + FrameworkPath + "' .",
...
public class RedpointEOSPlatform_Android : IRedpointEOSPlatform
{
	...
	/// <summary>
	/// Copied from AndroidToolChain since this isn't surfaced anywhere in the target when the modules are being
	/// set up.
	/// </summary>
	private List<string> ParseArchitectures(ModuleRules Module, ReadOnlyTargetRules Target)
	{
		List<string> AdditionalArches = null;
...
#else
            if (File.Exists(Path.Combine(Module.EngineDirectory, "Build", "InstalledBuild.txt")))
#endif
			{
	...
	public void ConfigureModule(string SdkBase, string ModuleBase, ModuleRules Module, ReadOnlyTargetRules Target)
	{
		if (ParseArchitectures(Module, Target).Contains("-x64"))
		{
			Console.WriteLine("error: The EOS SDK does not support the x86_64 architecture on Android. Only ARM builds are supported. Please untick the 'Support x86_64 [aka x64]' option in Project Settings.");
			throw new InvalidOperationException("The EOS SDK does not support the x86_64 architecture on Android. Only ARM builds are supported. Please untick the 'Support x86_64 [aka x64]' option in Project Settings.");
		}

		var IsSplitSDK = Directory.Exists(Path.Combine(SdkBase, "Bin", "Android", "static-stdc++"));
		string ArmEABILib, Arm64Lib, AARPath, AARName;
		if (!IsSplitSDK)
		{
...
#if UE_5_1_OR_LATER
		ArmEABILib = Path.Combine(SdkBase, "Bin", "Android", "static-stdc++", "libs", "armeabi-v7a", "libEOSSDK.so");
		Arm64Lib = Path.Combine(SdkBase, "Bin", "Android", "static-stdc++", "libs", "arm64-v8a", "libEOSSDK.so");
		AARPath = Path.Combine(SdkBase, "Bin", "Android", "static-stdc++", "aar");
		AARName = "Bin/Android/static-stdc++/aar";
#else
        ArmEABILib = Path.Combine(SdkBase, "Bin", "Android", "dynamic-stdc++", "libs", "armeabi-v7a", "libEOSSDK.so");
        Arm64Lib = Path.Combine(SdkBase, "Bin", "Android", "dynamic-stdc++", "libs", "arm64-v8a", "libEOSSDK.so");
        AARPath = Path.Combine(SdkBase, "Bin", "Android", "dynamic-stdc++", "aar");
        AARName = "Bin/Android/dynamic-stdc++/aar";
#endif

		string includeBase;
		if (Directory.Exists(Path.Combine(SdkBase, "Include")))
		{
			includeBase = Path.Combine(SdkBase, "Include");
		}
		else
		{
			includeBase = Path.Combine(SdkBase, "Bin", "Android", "include");
		}
		Module.PublicIncludePaths.Add(includeBase);
		Module.ExternalDependencies.Add(Path.Combine(includeBase, "eos_version.h"));
		Module.PublicAdditionalLibraries.Add(ArmEABILib);
		Module.PublicAdditionalLibraries.Add(Arm64Lib);
...
#if UE_5_0_OR_LATER
		var BaseTheme = "UnrealBaseTheme";
#else
        var BaseTheme = "UE4BaseTheme";
#endif
		var UPLTemplate = @"
...
<root xmlns:android=""http://schemas.android.com/apk/res/android"">
    <init>
        ...
        <!-- If we are packaging for Oculus Quest 2, do not enable any of the Google Play Billing features. -->
    ...
    <baseBuildGradleAdditions>
        ...
        <insertValue value=""dirs '__SDK_PATH__/__AAR_NAME__'""/>
        <insertNewline/>
        <insert>
                        }
                    }
    ...
    <androidManifestUpdates>
        <addAttribute tag=""application"" name=""android:theme"" value=""@style/" + BaseTheme + @""" />
	...
	<prebuildCopies>
        <if condition=""bPackageForOculusMobile"">
			<false>
		        ...
		        <copyDir src=""$S(PluginDir)/../OnlineSubsystemRedpointGoogle/Private/Java"" dst=""$S(BuildDir)/src/games/redpoint"" />
            </false>
        </if>
	</prebuildCopies>
...
";
		...
		foreach (var file in Directory.GetFiles(AARPath, "*.aar"))
		{
			DepsEntries.Add("            dependencies.implementation(name: '" + Path.GetFileNameWithoutExtension(file) + "', ext: 'aar')");
		}

		var UPLContent = UPLTemplate
			.Replace("__SDK_PATH__", SdkBase.Replace('\\', '/'))
			.Replace("__AAR_NAME__", AARName)
			.Replace("__DEPS_LIST__", string.Join("\n", DepsEntries));
		...
		RedpointEOSSDK.EnsureFilesExist(new[] {
			Path.Combine(includeBase, "eos_version.h"),
			ArmEABILib,
			Arm64Lib,
...
public class RedpointEOSSDK : ModuleRules
{
	...
	//
	// NOTE: You must also update the following files when adding new SDKs:
	// - ProcessProjectAfterStaging.ps1
	// - ProcessProjectForAntiCheat.ps1
	...
	public RedpointEOSSDK(ReadOnlyTargetRules Target) : base(Target)
	{
		...
		if (EOSSDKFullVersion.Contains("-hf-"))
		{
			...
			var HotfixVersion = EOSSDKFullVersion.Substring(EOSSDKFullVersion.IndexOf("-hf-") + "-hf-".Length);
			int HotfixVersionInt = 0;
			if (!int.TryParse(HotfixVersion, out HotfixVersionInt))
			{
		...
		{
			bool bHasEOSBuildPlatformNameOverride = false;
	...
	private static string LocateSdkDirectory(ModuleRules Module, ReadOnlyTargetRules Target, string ModuleBase, string PlatformSuffix, RedpointEOSSDKVersionConstraints Constraints, out string EOSSDKFullVersionOut)
	{
		...
		foreach (var EOSSDKFullVersion in EOSSDKSupportedVersions)
		{
			...
			else if (System.IO.Path.DirectorySeparatorChar == '\\')
			{
				CandidateDirectories = new string[]
				{
                    ...
                    // Where the SDK has been extracted into C:\ProgramData\EOS-SDK-<version>\, but the SDK has a weird extra subdirectory (1.15.2.1 onwards for some platforms).
                    Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData), "EOS-SDK" + PlatformSuffix + "-" + EOSSDKFullVersion, "EOS-SDK" + PlatformSuffix + "-" + EOSSDKFullVersion, "SDK"),
                    ...
                    // Where the SDK has been extracted into the user's downloads folder, but the SDK has a weird extra subdirectory (1.15.2.1 onwards for some platforms).
                    Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "Downloads", "EOS-SDK" + PlatformSuffix + "-" + EOSSDKFullVersion, "EOS-SDK" + PlatformSuffix + "-" + EOSSDKFullVersion, "SDK"),
                    // Where the SDK has been extracted into C:\EOS-SDK-<version>\
                    // @note: Path.Combine is broken for SYSTEMDRIVE ('C:') as the base path in .NET Framework. Just manually
                    // combine the paths for both .NET Framework and .NET Core since there's no other consistent way to use
                    // the API across the different .NET versions.
                    Environment.GetEnvironmentVariable("SYSTEMDRIVE").TrimEnd('\\') + "\\EOS-SDK" + PlatformSuffix + "-" + EOSSDKFullVersion + "\\SDK",
				};
			}
			else
			{
				CandidateDirectories = new string[]
				{
                    ...
                    // Where the SDK has been extracted into the user's downloads folder, but the SDK has a weird extra subdirectory (1.15.2.1 onwards for some platforms).
                    Path.Combine(Environment.GetEnvironmentVariable("HOME"), "Downloads", "EOS-SDK" + PlatformSuffix + "-" + EOSSDKFullVersion, "EOS-SDK" + PlatformSuffix + "-" + EOSSDKFullVersion, "SDK"),
				};
			}
		...
		{
			Console.WriteLine("ERROR: The ONLINE_SUBSYSTEM_EOS_FORCE_SDK_VERSION (" + ForcedSDKVersion + ") is incorrectly set, as it did not match any of the supported EOS SDK versions.");
		}
