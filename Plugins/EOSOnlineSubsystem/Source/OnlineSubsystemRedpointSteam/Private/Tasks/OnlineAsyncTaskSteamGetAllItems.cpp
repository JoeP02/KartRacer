...
void FOnlineAsyncTaskSteamGetAllItems::Finalize()
{
    ...
    {
        if (!ItemDefinitionsMap.Contains(Items[i].m_iDefinition))
        {
            UE_LOG(
                LogRedpointSteam,
                Warning,
                TEXT("Ignoring item with ID '%llu' and definition ID '%d' because we could not fetch metadata about "
                     "this definition from the Steamworks API."),
                Items[i].m_itemId,
                Items[i].m_iDefinition);
            continue;
        }
