    ...
    std::function<void()> OnDone)
{
    ...
    {
        UE_LOG(LogRedpointEOSTests, Verbose, TEXT("All invites restored, rejecting remaining invitations..."));
    ...
    const FOnDone &OnDone)
{
    for (const auto &KV : this->Instances)
    {
        ...
        UE_LOG(LogRedpointEOSTests, Warning, TEXT("Unable to reject pending invitation, expect party tests to fail"));
    }

    // Finally call OnDone.
    UE_LOG(LogRedpointEOSTests, Verbose, TEXT("All remaining invitations rejected."));
    OnDone();
}
    ...
    const FOnDone &OnDone)
{
    for (auto KV : this->ConnectedPartiesByUser)
    {
        ...
        UE_LOG(LogRedpointEOSTests, Warning, TEXT("Unable to leave party, expect party tests to fail"));
    }
