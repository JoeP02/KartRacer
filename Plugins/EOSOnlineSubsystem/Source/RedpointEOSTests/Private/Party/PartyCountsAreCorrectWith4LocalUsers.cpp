...
public:
    ...
    TMap<int32, FTSTicker::FDelegateHandle> InviteTimeoutHandle;
    ...
    FOnlineSubsystemEOS_OnlinePartyInterface_PartyCountsAreCorrectWith4LocalUsers_Manager(
        ...
        const FMultiplayerScenarioInstance &InClient1,
        const FMultiplayerScenarioInstance &InClient2,
        const FMultiplayerScenarioInstance &InClient3,
        const std::function<void()> &InOnDone)
    {
        this->T = InT;
        this->Host = InHost;
        this->Client1 = InClient1;
        this->Client2 = InClient2;
        this->Client3 = InClient3;
...
void FOnlineSubsystemEOS_OnlinePartyInterface_PartyCountsAreCorrectWith4LocalUsers_Manager::Start_SendInviteToClients()
{
    ...
    UE_LOG(LogRedpointEOSTests, Log, TEXT("Sending invitation to client 1"));
    ...
    UE_LOG(LogRedpointEOSTests, Log, TEXT("Sending invitation to client 2"));
    ...
    UE_LOG(LogRedpointEOSTests, Log, TEXT("Sending invitation to client 3"));
...
void FOnlineSubsystemEOS_OnlinePartyInterface_PartyCountsAreCorrectWith4LocalUsers_Manager::
    Start_RestoreInvitesOnClient(
        ...
        int32 ClientNum)
{
    UE_LOG(LogRedpointEOSTests, Log, TEXT("[Diagnostics] Restoring invites on client %d"), ClientNum);
        ...
        int32 ClientNum)
{
    ...
    {
        UE_LOG(LogRedpointEOSTests, Log, TEXT("[Diagnostics] Started an invite timeout for client %d"), ClientNum);
        ...
        int32 ClientNum)
{
    ...
    {
        UE_LOG(LogRedpointEOSTests, Verbose, TEXT("Ignoring invite because party ID didn't match the expected party ID"));
        return;
    }
    if (LocalUserId != *InClient.UserId)
    {
        UE_LOG(LogRedpointEOSTests, Verbose, TEXT("Ignoring invite because this event is for a user other than our client"));
        return;
    }
    ...
    {
        FTSTicker::GetCoreTicker().RemoveTicker(this->InviteTimeoutHandle[ClientNum]);
        this->InviteTimeoutHandle.Remove(ClientNum);
    }

    UE_LOG(LogRedpointEOSTests, Log, TEXT("[Diagnostics] Retrieving invites on client %d"), ClientNum);
    ...
    UE_LOG(LogRedpointEOSTests, Log, TEXT("[Diagnostics] Processing invites on client %d"), ClientNum);
...
{
    this->Client1PartySystemWk.Pin()->AddOnPartyInviteReceivedExDelegate_Handle(
        FOnPartyInviteReceivedExDelegate::CreateSP(this, &TThisClass::Handle_ReceiveInviteOnClient, this->Client1, 1));
    this->Client2PartySystemWk.Pin()->AddOnPartyInviteReceivedExDelegate_Handle(
        FOnPartyInviteReceivedExDelegate::CreateSP(this, &TThisClass::Handle_ReceiveInviteOnClient, this->Client2, 2));
    this->Client3PartySystemWk.Pin()->AddOnPartyInviteReceivedExDelegate_Handle(
        FOnPartyInviteReceivedExDelegate::CreateSP(this, &TThisClass::Handle_ReceiveInviteOnClient, this->Client3, 3));
