...
public:
    ...
    FTSTicker::FDelegateHandle KickTimeoutHandle;
    ...
    Start_CreatePartyOnHost()
{
    ...
    UE_LOG(LogRedpointEOSTests, Log, TEXT("Creating party 1"));
    if (!T->TestTrue(
...
{
    UE_LOG(LogRedpointEOSTests, Log, TEXT("Registering event to handle member join"));
...
void FOnlineSubsystemEOS_OnlinePartyInterface_PartyExitedEventFiresWhenKickedFromParty_Manager::
    Handle_ReceiveInviteOnClient(const FUniqueNetId &LocalUserId, const IOnlinePartyJoinInfo &Invitation)
{
    ...
    const FOnlinePartyId &PartyId)
{
    ...
    FTSTicker::GetCoreTicker().RemoveTicker(this->KickTimeoutHandle);
        ...
        const EKickMemberCompletionResult Result)
{
    ...
    this->KickTimeoutHandle =
        FTSTicker::GetCoreTicker().AddTicker(FTickerDelegate::CreateSP(this, &TThisClass::Handle_KickTimeout), 5.0f);
}
