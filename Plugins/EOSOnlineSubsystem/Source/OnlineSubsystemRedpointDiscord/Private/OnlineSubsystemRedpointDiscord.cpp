...
bool FOnlineSubsystemRedpointDiscord::Init()
{
    ...
    this->Instance->SetLogHook(discord::LogLevel::Debug, [](discord::LogLevel level, const char *message) {
        ...
        case discord::LogLevel::Error:
            // Downgrade this error level; it happens on login and is unrelated
            // to any action that the developer takes.
            if (Message.Contains(TEXT("unknown variant `unknown`")))
            {
    ...
    this->IdentityImpl->RegisterEvents();
    this->AvatarImpl =
