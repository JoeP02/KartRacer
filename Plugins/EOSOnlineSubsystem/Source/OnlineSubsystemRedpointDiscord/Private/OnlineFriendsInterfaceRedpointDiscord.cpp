...
{
    // @note: For correctness, we need to fire the OnFriendsChangeDelegate event after we first switch from
    // NotLoggedIn to UsingLocalProfile in the identity interface. However, OnFriendsChangeDelegate already fires at
    // startup after we get the relationships for the first time, and we assume that Discord won't fire the OnRefresh
    // function until after it's also fired OnCurrentUserUpdate (it wouldn't really make sense for Discord to
    // refresh friends before telling us who the local user even is).
    //
    // If we end up in scenarios where developers can call ReadFriendsList super early and get no friends list (due
    // to the login status being NotLoggedIn) but somehow miss the OnFriendsChangeDelegate call below, then we'd need
    // to update the identity interface to also fire the OnFriendsChangeDelegate after it receives the current user
    ...
    const FOnReadFriendsListComplete &Delegate)
{
    ...
    if (this->Identity->GetLoginStatus(LocalUserNum) == ELoginStatus::NotLoggedIn)
    {
